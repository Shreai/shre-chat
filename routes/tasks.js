...
import { buildTaskCreateHeaders } from './task-intake-auth.js';
  ...
  return async function handleTaskRoute(req, res, url, { json, collectBody, rateLimit }) {
    ...
    if (url.pathname === '/api/tasks/create' && req.method === 'POST') {
      ...
      if (!taskRl.allowed)
        return json(
          res,
          { error: 'Too many task creation requests', retryAfter: taskRl.retryAfter },
          429,
        );
      ...
      try {
        ...
        const cleanTitle = String(title)
          ...
          .trim();
        if (!cleanTitle)
          return json(res, { error: 'title cannot be empty after sanitization' }, 400);
        ...
        // Route through the supported task create endpoint.
        const createPayload = {
          title: cleanTitle,
        ...
        const taskRes = await fetch(`${serviceUrl('shre-tasks')}/v1/tasks`, {
          method: 'POST',
          headers: buildTaskCreateHeaders(),
        ...
        if (!taskRes.ok) {
          ...
          return json(
            res,
            { error: 'Failed to create task', detail: errText.slice(0, 200) },
            taskRes.status,
          );
        }

        const result = await taskRes.json();
        const normalizedTaskId = result.objective_id || result.id || result.task_id;
        log.info('Task created from chat via intake', {
    ...
    if (url.pathname === '/api/issues/create' && req.method === 'POST') {
      ...
      if (!issueRl.allowed)
        return json(
          res,
          { error: 'Too many issue creation requests', retryAfter: issueRl.retryAfter },
          429,
        );
      ...
      try {
        ...
        if (!targetCompanyId)
          return json(res, { error: 'No company found — cannot create issue' }, 400);
        ...
        if (!issueRes.ok) {
          ...
          log.warn('Issue creation failed', {
            status: issueRes.status,
            error: errText.slice(0, 200),
          });
          return json(
            res,
            { error: 'Failed to create issue', detail: errText.slice(0, 200) },
            issueRes.status,
          );
        }

        const result = await issueRes.json();
        log.info('Issue created from chat', {
          issueId: result.id,
    ...
    if (url.pathname === '/api/tasks' && req.method === 'GET') {
      try {
        ...
        if (req.headers['x-workspace-id'])
          userHeaders['X-Workspace-Id'] = req.headers['x-workspace-id'];
        ...
        if (!taskRes.ok) {
          ...
          return json(
            res,
            { error: 'Failed to list tasks', detail: errText.slice(0, 200) },
            taskRes.status,
          );
        }
    ...
    if (url.pathname.match(/^\/api\/tasks\/[a-zA-Z0-9_-]+$/) && req.method === 'GET') {
      ...
      try {
        ...
        const taskRes = await fetch(
          `${serviceUrl('shre-tasks')}/v1/tasks/${encodeURIComponent(taskId)}`,
          {
            headers: {
              ...(svcToken ? { Authorization: `Bearer ${svcToken}` } : {}),
            },
    ...
    if (url.pathname.match(/^\/api\/tasks\/[a-zA-Z0-9_-]+$/) && req.method === 'PATCH') {
      ...
      try {
        ...
        if (!taskRes.ok) {
          ...
          return json(
            res,
            { error: 'Failed to update task', detail: errText.slice(0, 200) },
            taskRes.status,
          );
        }
    ...
    if (url.pathname.match(/^\/api\/tasks\/[a-zA-Z0-9_-]+\/retry$/) && req.method === 'POST') {
      ...
      try {
        const svcToken = process.env.SHRE_TASKS_TOKEN || '';
        const taskRes = await fetch(
          `${serviceUrl('shre-tasks')}/v1/tasks/${encodeURIComponent(taskId)}/retry`,
          {
            method: 'POST',
            headers: {
              ...(svcToken ? { Authorization: `Bearer ${svcToken}` } : {}),
        ...
        if (!taskRes.ok) {
          ...
          return json(
            res,
            { error: 'Failed to retry task', detail: errText.slice(0, 200) },
            taskRes.status,
          );
        }
