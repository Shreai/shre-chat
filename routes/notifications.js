...
const EVENT_FORMATTERS = {
  ...
  "fleet.agent.stuck": (data) => ({
    type: "fleet.agent.stuck",
    title: `Agent ${data.agentId || "unknown"} is stuck`,
    body: `Heartbeat missed ${data.missedCount} times. Task: ${data.taskId?.slice(0, 8) || "unknown"}`,
    source: "fleet",
  }),
  "fleet.agent.dead": (data) => ({
    type: "fleet.agent.dead",
    title: `Agent ${data.agentId || "unknown"} is dead`,
    body: `Heartbeat lost after ${data.missedCount} misses. Triggering recovery for task ${data.taskId?.slice(0, 8) || "unknown"}`,
    source: "fleet",
  }),
  "fleet.agent.recovered": (data) => ({
    type: "fleet.agent.recovered",
    title: `Agent ${data.agentId || "unknown"} recovered`,
    body: `Heartbeat resumed for task ${data.taskId?.slice(0, 8) || "unknown"}`,
    source: "fleet",
  }),
  "fleet.task.degraded": (data) => ({
    type: "fleet.task.degraded",
    title: `Fleet Task Degraded`,
    body: `Task ${data.taskId?.slice(0, 8)} failed to spawn on ${data.agent}. Error: ${data.error}`,
    source: "fleet",
  }),
  "fleet.agent.crash_unrecoverable": (data) => ({
    type: "fleet.agent.crash_unrecoverable",
    title: `Agent Crash Unrecoverable`,
    body: `Agent ${data.agentId} crashed permanently on task ${data.taskId?.slice(0, 8)}`,
    source: "fleet",
  }),
  "deploy.monitor.breach": (data) => ({
    type: "deploy.monitor.breach",
    title: `Deployment Breach: ${data.service}`,
    body: `Service ${data.service} breached health thresholds. Breaches: ${JSON.stringify(data.breaches)}`,
    source: "fleet",
  }),
  "fleet.code_quality": (data) => ({
    type: "fleet.code_quality",
    title: `Code Quality Scored: ${data.taskId?.slice(0, 8)}`,
    body: `Score: ${data.score}/100. Issues: ${data.issues || "none"}`,
    source: "checker",
  }),
  "fleet.done-gate.failed": (data) => ({
    type: "fleet.done-gate.failed",
    title: `Definition of Done Failed`,
    body: `Task ${data.taskId?.slice(0, 8)} did not meet completion criteria: ${data.reason}`,
    source: "checker",
  }),
  "fleet.verify.passed": (data) => ({
    type: "fleet.verify.passed",
    title: `Verification Passed`,
    body: `Agent ${data.agentId} successfully verified changes for task ${data.taskId?.slice(0, 8)}`,
    source: "verify",
  }),
  "fleet.verify.fix_created": (data) => ({
    type: "fleet.verify.fix_created",
    title: `Verification Failed - Fix Created`,
    body: `Verification failed for task ${data.taskId?.slice(0, 8)}. Ellie has created a fix and is retrying.`,
    source: "verify",
  }),
  "wave.started": (data) => ({
    type: "wave.started",
    title: `Wave ${data.index + 1} Started`,
    body: `Project ${data.projectId}: Processing ${data.taskCount} tasks.`,
    source: "orchestrator",
  }),
  "wave.completed": (data) => ({
    type: "wave.completed",
    title: `Wave ${data.index + 1} Completed`,
    body: `Project ${data.projectId}: Wave finished with ${data.successCount} successes.`,
    source: "orchestrator",
  }),
  "twin.divergence": (data) => ({
    type: "twin.divergence",
    title: `Twin Divergence Detected`,
    body: `Models diverged on task ${data.taskId?.slice(0, 8)}. Divergence score: ${data.divergence}. Review recommended.`,
    source: "twin",
  }),
