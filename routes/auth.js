...
/**
 * Best-effort sync so newly created/linked chat users also exist in MIB007.
 * This keeps cross-app access aligned without blocking login if MIB is down.
 * @param {{ userId?: string; workspaceId?: string; role?: string; email?: string; name?: string }} opts
 */
async function syncMembershipToMIB007(opts) {
  const workspaceId = opts.workspaceId || opts.userId;
  if (!opts.userId || !workspaceId) return;
  try {
    const { serviceUrl } = await import('shre-sdk/discovery');
    const mibUrl = serviceUrl('mib007');
    const res = await fetch(`${mibUrl}/api/auth/sync-membership`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Internal-Service': 'shre-auth',
      },
      body: JSON.stringify({
        userId: opts.userId,
        workspaceId,
        ...
        email: opts.email || '',
        name: opts.name || opts.email || opts.userId,
      }),
      signal: AbortSignal.timeout(5000),
    });
    if (!res.ok) {
      // Non-fatal: chat login should still succeed even if MIB is offline.
      return;
    }
  } catch {
...
function saveUsers(users) {
  try {
    writeVault(USERS_PATH, JSON.stringify(users, null, 2));
  } catch {
    /* best effort */
  }
}
...
function loadTrustedDevices() {
  try {
    return existsSync(TRUSTED_DEVICES_PATH)
      ? JSON.parse(readFileSync(TRUSTED_DEVICES_PATH, 'utf8'))
      : {};
  } catch {
    return {};
  }
...
function trustDevice(username) {
  ...
  devices[`${username}:${token}`] = {
    ...
    expiresAt: Math.floor(Date.now() / 1000) + TRUST_TTL,
  };
  saveTrustedDevices(devices);
  return token;
}
...
function generateOTP() {
  ...
  return String(
    (((bytes[0] << 24) | (bytes[1] << 16) | (bytes[2] << 8) | bytes[3]) >>> 0) % 1000000,
  ).padStart(6, '0');
}
...
function issueAuthToken(username, role = 'admin') {
  ...
  const sig = createHmac('sha256', authSigningKey)
...
export function verifyAuthToken(token) {
  ...
  try {
    ...
    const expected = createHmac('sha256', authSigningKey)
      .update(`${header}.${payload}`)
      .digest('base64url');
...
if (!existsSync(USERS_PATH)) {
  ...
  const defaultAdminPassword =
    ...
    (process.env.NODE_ENV === 'test' || process.env.VITEST
      ? 'test-admin-password'
      : (() => {
          throw new Error('SHRE_ADMIN_PASSWORD env var required');
        })());
  writeFileSync(
    USERS_PATH,
    JSON.stringify(
      {
        [process.env.SHRE_ADMIN_USER || 'admin']: {
          hash: hashPassword(defaultAdminPassword),
          role: 'admin',
          name: 'Admin',
        },
      },
...
export function registerAuthRoutes({ log }) {
  return async function handleAuthRoute(
    req,
    res,
    url,
    { json, rateLimit, authCookie, isJtiRevoked },
  ) {
    ...
    if (url.pathname === '/api/auth/login' && req.method === 'POST') {
      ...
      if (!rl.allowed) {
        ...
        return json(
          res,
          { error: 'Too many login attempts. Try again later.', retryAfter: rl.retryAfter },
          429,
        );
      }
      let body = '';
        ...
        try {
          ...
          try {
            ...
            if (!authRes.ok) {
              ...
              if (data.code === 'NO_ACCOUNT' || authRes.status === 404) {
                return json(res, { ...data, shouldSuggestSignup: true }, 404);
              }
              if (shouldFallbackToLocalAuth(authRes.status)) {
                throw new Error(data.error || `shre-auth unavailable (${authRes.status})`);
              }
            ...
            if (!data.requiresWorkspaceSelection) {
              ...
              if (localUser?.twoFactor && localUser.email) {
                ...
                if (trustToken && isDeviceTrusted(username, trustToken)) {
                  ...
                  res.setHeader(
                    'Set-Cookie',
                    authCookie('shre_token', data.token, AUTH_TOKEN_TTL, req),
                  );
                ...
                const masked = localUser.email.replace(
                  /^(.{1,2})(.*)(@.*)$/,
            ...
            if (data.token) {
              ...
              syncMembershipToMIB007({
                userId: data.user?.id || data.user?.username,
                workspaceId: data.workspace?.id,
                role: data.workspace?.role || data.user?.role || 'member',
                email: data.user?.email || data.user?.username || username,
            ...
            log.warn('[auth] shre-auth unavailable, falling back to local auth', {
            ...
            if (user.twoFactor && user.email) {
              ...
              if (trustToken && isDeviceTrusted(username, trustToken)) {
                ...
                return json(res, {
                  token,
                  user: { username, name: user.name || username, role: user.role || 'admin' },
                });
              }
              const code = generateOTP();
    ...
    if (url.pathname === '/api/auth/bootstrap-status' && req.method === 'GET') {
      try {
        const { serviceUrl } = await import('shre-sdk/discovery');
        const authUrl = serviceUrl('shre-auth');
        const authRes = await fetch(`${authUrl}/v1/auth/bootstrap-status`, {
          signal: AbortSignal.timeout(5000),
        });
        const data = await authRes.json();
        return json(res, data, authRes.status);
      } catch (err) {
        log.warn('[auth] bootstrap-status proxy failed', { error: err?.message });
        return json(res, { hasWorkspaces: true, hasUsers: true, shouldStartInSignup: false }, 200);
      }
    }

    if (url.pathname === '/api/auth/account-exists' && req.method === 'GET') {
      const username = url.searchParams.get('username')?.trim();
      if (!username) {
        return json(res, { exists: false }, 400);
      }
      try {
        const { serviceUrl } = await import('shre-sdk/discovery');
        const authUrl = serviceUrl('shre-auth');
        const authRes = await fetch(
          `${authUrl}/v1/auth/account-exists?username=${encodeURIComponent(username)}`,
          {
            signal: AbortSignal.timeout(5000),
          },
        );
        ...
        log.warn('[auth] account-exists proxy failed', { error: err?.message });
        return json(res, { error: 'Auth lookup unavailable', code: 'LOOKUP_UNAVAILABLE' }, 503);
      }
    }
      ...
      req.on('end', async () => {
        try {
          ...
          syncMembershipToMIB007({
            ...
            name: data.user?.name || parsed.name || parsed.email,
          }).catch(() => {});
          json(res, data);
        } catch (err) {
    ...
    if (url.pathname === '/api/auth/verify-2fa' && req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          ...
          if (!otp)
            return json(res, { error: 'No pending verification. Please log in again.' }, 400);
          ...
          if (!timingSafeEqual(inputBuf, expectedBuf)) {
            ...
            return json(
              res,
              { error: `Invalid code. ${5 - otp.attempts} attempts remaining.` },
              401,
            );
          }
          ...
          if (platformData) {
            ...
            const cookieHeaders = [
              authCookie('shre_token', platformData.token, AUTH_TOKEN_TTL, req),
            ];
            if (shouldTrust) {
            ...
            syncMembershipToMIB007({
              userId: platformData.user?.id || platformData.user?.username || username,
              workspaceId: platformData.workspace?.id,
              role: platformData.workspace?.role || platformData.user?.role || 'member',
              email: platformData.user?.email || username,
              name: platformData.user?.name || username,
            }).catch(() => {});
    ...
    if (url.pathname === '/api/auth/check' && req.method === 'GET') {
      ...
      try {
        ...
        if (valData.valid && valData.claims) {
          ...
          return json(res, {
            ...
            workspaces: (valData.claims.workspaceIds || []).map((workspaceId) => ({
              id: workspaceId,
              name: workspaceId,
              role:
                workspaceId === valData.claims.activeWorkspaceId ? valData.claims.role : 'member',
            })),
          });
        }
      } catch {
      ...
      json(res, {
        authenticated: true,
        user: { username: claims.sub, name: user?.name || claims.sub, role: claims.role },
        workspace: claims.activeWorkspaceId
          ? {
              id: claims.activeWorkspaceId,
              name: claims.activeWorkspaceId,
              role: claims.role,
            }
          : null,
        workspaces: claims.activeWorkspaceId
          ? [
              {
                id: claims.activeWorkspaceId,
                name: claims.activeWorkspaceId,
      ...
      try {
        ...
        if (valRes.ok) {
          ...
          return json(res, {
            ...
            workspace: valData.claims.activeWorkspaceId
              ? {
                  id: valData.claims.activeWorkspaceId,
                  name: valData.claims.activeWorkspaceName || valData.claims.activeWorkspaceId,
                  role: valData.claims.role || 'user',
                }
              : undefined,
    ...
    if (url.pathname === '/api/auth/refresh' && req.method === 'POST') {
      ...
      try {
        ...
        if (!upstream.ok || !body?.token) {
          // If the current token is a valid local token, preserve continuity
          // with a local refresh even when shre-auth rejects the token.
          const claims = verifyAuthToken(currentToken);
          if (claims?.username) {
            const token = issueAuthToken(claims.username, claims.role || 'admin');
            if (token) {
              res.setHeader('Set-Cookie', authCookie('shre_token', token, 8 * 60 * 60, req));
              return json(res, { token });
            }
          }
          return json(
            res,
            body?.error ? body : { error: 'Refresh failed' },
            upstream.status || 401,
          );
        }
        ...
        // Preserve the local session if the auth service is down.
        const claims = verifyAuthToken(currentToken);
        if (claims?.username) {
          const token = issueAuthToken(claims.username, claims.role || 'admin');
          if (token) {
      ...
      req.on('end', () => {
        try {
          ...
          if (!code || typeof code !== 'string')
            return json(res, { verified: false, error: 'Code required' }, 400);
          ...
          if (!existsSync(scriptPath))
            return json(res, { verified: false, error: 'Verification unavailable' }, 503);
          ...
          child.on('error', () =>
            json(res, { verified: false, error: 'Verification failed' }, 500),
          );
        } catch {
          return json(res, { verified: false, error: 'Invalid request' }, 400);
        }
      });
      return true;
    }
      ...
      req.on('end', async () => {
        try {
          ...
          try {
            ...
            log.warn('[auth] Passport service unavailable, using local auth fallback', {
              error: err?.message,
            });
...
/**
 * Treat upstream auth 5xx failures as transient so local auth can take over.
 * This keeps login usable when shre-auth or its backing DB is degraded.
 * @param {number} status
 * @returns {boolean}
 */
export function shouldFallbackToLocalAuth(status) {
  return status >= 500;
}
