...
const INDUSTRIES = [
  ...
  "Consulting", "Education", "Non-profit", "Other",
];
...
export function OnboardingView({ profile, onComplete, onSkip }: Props) {
  ...
  const steps = [
    ...
    <div key="about" className="space-y-5">
      <div className="text-center">
        ...
        <h2 className="text-xl font-semibold" style={{ color: "var(--c-text-1)" }}>Welcome! Let's get to know you</h2>
        <p className="text-sm mt-1" style={{ color: "var(--c-text-4)" }}>This helps your AI assistants personalize their help</p>
      </div>
      <div className="space-y-3">
        <Field label="Your name" value={p.name} onChange={(v) => update({ name: v })} placeholder="e.g., Nir" />
        <Field label="Your role / title" value={p.role} onChange={(v) => update({ role: v })} placeholder="e.g., CEO, Store Manager, Developer" />
        <Field label="Short bio (optional)" value={p.bio} onChange={(v) => update({ bio: v })} placeholder="What do you do? What drives you?" multiline />
      </div>
    </div>,
    ...
    <div key="business" className="space-y-5">
      <div className="text-center">
        <div className="text-4xl mb-3">🏪</div>
        <h2 className="text-xl font-semibold" style={{ color: "var(--c-text-1)" }}>Tell us about your business</h2>
      </div>
      <div className="space-y-3">
    ...
    <div key="goals" className="space-y-5">
      <div className="text-center">
        <div className="text-4xl mb-3">🎯</div>
        <h2 className="text-xl font-semibold" style={{ color: "var(--c-text-1)" }}>Goals & Challenges</h2>
        <p className="text-sm mt-1" style={{ color: "var(--c-text-4)" }}>What are you trying to achieve? What's getting in the way?</p>
      </div>
      <div className="space-y-4">
        <TagInput label="Top goals" placeholder="e.g., Increase online orders" value={goalInput} onChange={setGoalInput} items={p.business.goals}
          onAdd={(v) => updateBiz({ goals: [...p.business.goals, v] })}
          onRemove={(i) => updateBiz({ goals: p.business.goals.filter((_, j) => j !== i) })} />
        <TagInput label="Biggest challenges" placeholder="e.g., Staff scheduling" value={challengeInput} onChange={setChallengeInput} items={p.business.challenges}
          onAdd={(v) => updateBiz({ challenges: [...p.business.challenges, v] })}
          onRemove={(i) => updateBiz({ challenges: p.business.challenges.filter((_, j) => j !== i) })} />
        <TagInput label="Tools you use" placeholder="e.g., RapidRMS, Square, DoorDash" value={toolInput} onChange={setToolInput} items={p.business.tools}
    ...
    <div key="prefs" className="space-y-5">
      <div className="text-center">
        ...
        <h2 className="text-xl font-semibold" style={{ color: "var(--c-text-1)" }}>How should I communicate?</h2>
      </div>
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--c-text-3)" }}>Communication style</label>
        ...
        <Toggle label="Show pending tasks on greeting" checked={p.preferences.showTasksOnGreeting} onChange={(v) => updatePrefs({ showTasksOnGreeting: v })} />
        <Toggle label="Notify when agents finish background work" checked={p.preferences.notifyOnComplete} onChange={(v) => updatePrefs({ notifyOnComplete: v })} />
        <Toggle label="Enable floating chat bubble" checked={p.preferences.floatingChat} onChange={(v) => updatePrefs({ floatingChat: v })} />
      </div>
    </div>,
  ];
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: "var(--c-bg-1)" }}>
      <div className="w-full max-w-lg">
        ...
        <div className="rounded-2xl p-6" style={{ background: "var(--c-bg-2)", border: "1px solid var(--c-border-1)" }}>
          {steps[step]}

          <div className="flex items-center justify-between mt-6 pt-4" style={{ borderTop: "1px solid var(--c-border-2)" }}>
            {step > 0 ? (
              <button onClick={() => setStep(step - 1)} className="text-sm px-4 py-2 rounded-lg" style={{ color: "var(--c-text-3)" }}>Back</button>
            ) : (
...
function Field({ label, value, onChange, placeholder, multiline }: {
  ...
  const style = {
    background: "var(--c-bg-3)", border: "1px solid var(--c-border-2)", color: "var(--c-text-1)",
    outline: "none", width: "100%", padding: "8px 12px", borderRadius: "8px", fontSize: "13px",
  };
  return (
    <div>
...
function Chip({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className="px-2.5 py-1 rounded-full text-xs transition-all" style={{
    ...
    <div>
      ...
      <div className="flex gap-2">
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" && value.trim()) { onAdd(value.trim()); onChange(""); e.preventDefault(); }
          }}
          className="flex-1"
          style={{ background: "var(--c-bg-3)", border: "1px solid var(--c-border-2)", color: "var(--c-text-1)", outline: "none", padding: "6px 10px", borderRadius: "8px", fontSize: "13px" }}
        />
        <button
          onClick={() => { if (value.trim()) { onAdd(value.trim()); onChange(""); } }}
      ...
      {items.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-2">
          {items.map((item, i) => (
            <span key={i} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs" style={{ background: "rgba(99,102,241,0.12)", color: "var(--c-accent)" }}>
              {item}
              <button onClick={() => onRemove(i)} className="opacity-60 hover:opacity-100">&times;</button>
            </span>
          ))}
        </div>
      )}
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <label className="flex items-center justify-between cursor-pointer">
      ...
      >
        <div className="absolute top-0.5 h-4 w-4 rounded-full bg-white transition-transform" style={{ left: checked ? "22px" : "2px" }} />
      </div>
    </label>
  );
}
