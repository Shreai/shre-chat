...
export function TaskTimelineView() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (cancelled) return;
      if (!result) { setError("Could not load tasks. Is shre-tasks running?"); setLoading(false); return; }
  ...
  const agents = useMemo(() => Array.from(new Set(tasks.map(t => t.agent_id).filter(Boolean) as string[])), [tasks]);
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden" style={{ background: "var(--c-bg-1)" }}>
      <div className="px-4 py-3 flex items-center gap-2 justify-between" style={{ borderBottom: "1px solid var(--c-border-1)" }}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold" style={{ color: "var(--c-text-1)" }}>Task Timeline</span>
          <span className="text-[10px] font-mono" style={{ color: "var(--c-text-5)" }}>via shre-tasks</span>
        </div>
        <DateRangeSelector value={range} onChange={setRange} />
      </div>

      {/* Filters */}
      <div className="px-4 py-2 flex items-center gap-3 flex-wrap" style={{ borderBottom: "1px solid var(--c-border-2)" }}>
        <div className="flex items-center gap-1">
          <span className="text-[10px]" style={{ color: "var(--c-text-5)" }}>Status:</span>
          {statuses.map(s => (
            <button
              key={s}
        ...
        {agents.length > 0 && (
          <div className="flex items-center gap-1">
            <span className="text-[10px]" style={{ color: "var(--c-text-5)" }}>Agent:</span>
            <select
              value={agentFilter}
      ...
      <div className="flex-1 overflow-auto p-4">
        ...
        {!loading && ganttTasks.length > 0 && (
          <div className="rounded-lg p-3 overflow-x-auto" style={{ background: "var(--c-bg-2)", border: "1px solid var(--c-border-2)" }}>
