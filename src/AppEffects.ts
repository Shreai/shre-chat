...
import type { Session, View, Theme, ThemeCustom, AppActions, FeedEntry, UploadedFile } from "./store";
import {
  loadSessions,
  syncWithServer,
  saveActiveSession,
...
export function useInitEffects(
  ...
  useEffect(() => {
    ...
    }).catch(() => { void 0; }).finally(() => setSyncing(false));
    ...
    const persistStreamIfActive = () => {
      if (streamingRef.current && streamTextRef.current.trim() && activeSessionId) {
        ...
        if (idx >= 0) {
          ...
          try { localStorage.setItem("shre-sessions", JSON.stringify(sessions)); } catch (err) { console.debug("stream persist quota", err); }
        }
      }
    };
...
export function useCrossTabSync(
  ...
  useEffect(() => {
    const handler = (e: StorageEvent) => {
      ...
      } catch (err) { console.debug("cross-tab storage event JSON parse", err); }
...
export function useUpdateSessions(
  ...
  return useCallback((fn: (prev: Session[]) => Session[]) => {
    setSessions((prev) => {
      const next = fn(prev);
