...
function prepareStatements(db: InstanceType<typeof Database>) {
  ...
  const stmtGetAll = db.prepare(
    `SELECT id, title, agent_id, pinned, tags, system_prompt, parent_id, created_at, updated_at, user_id, tenant_id FROM chat_sessions WHERE user_id = ? ORDER BY updated_at DESC LIMIT 100`,
  );
  ...
  `);
  const stmtListDeleted = db.prepare(
    `SELECT id, title, agent_id, deleted_at, deleted_by FROM deleted_sessions WHERE user_id = ? ORDER BY deleted_at DESC LIMIT 50`,
  );
  ...
  `);
  const stmtRemoveFromTrash = db.prepare(
    `DELETE FROM deleted_sessions WHERE id = ? AND user_id = ?`,
  );
...
describe('Tenant Isolation — Chat Sessions', () => {
  ...
  beforeEach(() => {
    ...
    stmts.upsertSession(
      { id: 'alice-1', title: 'Alice chat 1', messages: [{ role: 'user', content: 'hello' }] },
      'alice',
      'tenant-a',
    );
    stmts.upsertSession(
      { id: 'alice-2', title: 'Alice chat 2', messages: [] },
      'alice',
      'tenant-a',
    );
    stmts.upsertSession(
      { id: 'bob-1', title: 'Bob chat 1', messages: [{ role: 'user', content: 'hi' }] },
      'bob',
      'tenant-b',
    );
  ...
  it("user cannot overwrite another user's session via upsert", () => {
    ...
    stmts.upsertSession(
      { id: 'bob-1', title: 'HACKED', messages: [{ role: 'user', content: 'pwned' }] },
      'alice',
      'tenant-a',
    );
  ...
  it('orphan migration claims all system sessions for the first authenticated user', () => {
    ...
    const orphanCount = db
      .prepare("SELECT COUNT(*) as c FROM chat_sessions WHERE user_id = 'system'")
      .get() as any;
