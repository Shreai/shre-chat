...
describe("chunkIntoSentences", () => {
  ...
  it("returns empty array for empty messages", () => {
    ...
    expect(chunkIntoSentences(null as any)).toEqual([]);
    expect(chunkIntoSentences(undefined as any)).toEqual([]);
  });
...
describe("scoreRelevance", () => {
  ...
  it("returns empty array for empty input", () => {
    ...
    expect(scoreRelevance(null as any, "query")).toEqual([]);
  });

  it("handles empty query — returns chunks with base scores", () => {
    const chunks = [
      { text: "Some data", role: "assistant", msgIndex: 0, charCount: 9 },
    ];
...
describe("assembleContext", () => {
  ...
  it("handles empty input", () => {
    ...
    expect(assembleContext(null as any)).toEqual({ relevant: false, context: "" });
  });
