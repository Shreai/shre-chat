...
import { createMockLogger, createMockReq, createMockRes, createJsonHelper, createCollectBodyHelper, createRateLimitHelper, getJsonResponse, } from './route-test-helpers';
...
let handleTask;
beforeAll(() => {
    handleTask = registerTaskRoutes({ log: log });
});
beforeEach(() => {
    fetchMock.mockReset();
});
/** Helper to POST a task creation request */
async function createTask(body, rateLimitHelper) {
...
function mockTaskServiceSuccess(taskId = 'task-123', title = 'Test task') {
    fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({ id: taskId, title }), { status: 200 }));
}
