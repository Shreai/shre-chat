...
import { checkGateway, generateAITitle, setAgent, listSessions, fetchSessionMessages, } from '../router-client';
// ── Setup / Teardown ─────────────────────────────────────────────────
beforeEach(() => {
    fetchMock.mockReset();
});
...
describe('checkGateway', () => {
    it('returns true when gateway responds (even non-200)', async () => {
        fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({ error: 'bad model' }), { status: 400 }));
...
describe('generateAITitle', () => {
    ...
    it('returns title from output_text fallback', async () => {
        fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({ output_text: 'Quick Chat Title' }), { status: 200 }));
