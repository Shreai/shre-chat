...
function buildHistory(allMessages: ChatMessage[], replyToIndex: number | null): ChatMessage[] {
  return replyToIndex !== null ? allMessages.slice(0, replyToIndex + 1) : allMessages;
}
...
const conversation: ChatMessage[] = [
  ...
  {
    role: 'assistant',
    content: 'Decorators are experimental metadata annotations.',
    timestamp: 1007,
  },
];
...
describe('Voice-friendly context anchoring keywords', () => {
  ...
  const continuePattern =
    /^(continue|keep\s+going|go\s+on|go\s+ahead|finish\s+(this|that|it)|carry\s+on|resume|pick\s+up\s+where|and\s*\??|then\s*\??|next\s*\??|keep\s+going\s+with\s+(this|that)|finish\s+what\s+you\s+(were|started)|where\s+were\s+we)$/i;
  ...
  function matchesStatus(text: string): boolean {
    return statusPattern.test(stripFiller(text));
  }
  function matchesContinue(text: string): boolean {
    return continuePattern.test(stripFiller(text));
  }
  function matchesRecall(text: string): boolean {
    return recallPattern.test(stripFiller(text));
  }
