...
function mockDigestSuccess(
  digest: any = { pendingTasks: 5, completedToday: 3, activeProjects: 2, blockedTasks: 1 },
) {
...
describe('Voice command — AI classification fallback', () => {
  it('uses AI fallback when no regex matches, classifies as task_create', async () => {
          ...
          choices: [
            {
              message: {
                content:
                  '{"intent":"task_create","title":"pick up dry cleaning","priority":"medium"}',
              },
            },
          ],
        }),
    ...
    fetchMock.mockResolvedValueOnce(
      new Response(JSON.stringify({ id: 'ai-task-1' }), { status: 200 }),
    );
