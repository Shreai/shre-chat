...
function prepareStatements(db) {
  ...
  `);
    ...
    function upsertSession(s, userId, tenantId = 'default') {
        stmtUpsert.run(s.id, s.title || 'New chat', s.agentId || 'main', JSON.stringify(s.messages || []), s.pinned ? 1 : 0, JSON.stringify(s.tags || []), s.systemPrompt || null, s.parentId || null, s.createdAt || Date.now(), s.updatedAt || Date.now(), userId, tenantId);
    }
    return {
        stmtUpsert,
        stmtGetAll,
...
describe('Tenant Isolation — Chat Sessions', () => {
    ...
    it('user can only list their own sessions', () => {
        ...
        const aliceIds = aliceSessions.map((r) => r.id);
    ...
    it("user cannot read another user's session by ID", () => {
        ...
        expect(bobResult.title).toBe('Bob chat 1');
    });
    ...
    it('soft delete respects user scope', () => {
        ...
        expect(bobTrash[0].id).toBe('bob-1');
    ...
    it('upsert stamps user_id and tenant_id correctly', () => {
        ...
        expect(charlieResult.user_id).toBe('charlie');
        expect(charlieResult.tenant_id).toBe('tenant-c');
        // Others can't
    ...
    it('sync only returns sessions for the requesting user', () => {
        ...
        const allIds = aliceRows.map((r) => r.id);
    ...
    it("user cannot overwrite another user's session via upsert", () => {
        ...
        if (bobSession) {
            ...
            expect(bobSession.title).toBe('Bob chat 1');
        }
        else {
            ...
            expect(aliceSession.user_id).toBe('alice');
        }
    });
