...
import { createMockLogger, createMockReq, createMockRes, createJsonHelper, createCollectBodyHelper, getJsonResponse, } from './route-test-helpers';
// Mock shre-sdk before importing the route module
vi.mock('shre-sdk', () => ({
    serviceUrl: (name) => `http://mock-${name}:9999`,
    infraUrl: (name) => `http://mock-${name}:9999`,
}));
// Mock global fetch
...
async function voiceCommand(prompt) {
...
function mockTaskListSuccess(tasks = []) {
    fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({ tasks }), { status: 200 }));
}
/** Mock a digest response */
function mockDigestSuccess(digest = { pendingTasks: 5, completedToday: 3, activeProjects: 2, blockedTasks: 1 }) {
    ...
    it('uses AI fallback when no regex matches, classifies as task_create', async () => {
        ...
        fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({
            choices: [
                {
                    message: {
                        content: '{"intent":"task_create","title":"pick up dry cleaning","priority":"medium"}',
                    },
                },
            ],
        }), { status: 200 }));
