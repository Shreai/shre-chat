...
function issueTestToken(username, role = 'admin', signingKey = TEST_SIGNING_KEY) {
...
function issueExpiredToken(username, signingKey = TEST_SIGNING_KEY) {
    const header = Buffer.from(JSON.stringify({ alg: 'HS256', typ: 'JWT' })).toString('base64url');
    const now = Math.floor(Date.now() / 1000);
    const payload = Buffer.from(JSON.stringify({
        sub: username,
...
function verifyTestToken(token, signingKey = TEST_SIGNING_KEY) {
    if (!token)
        return null;
    try {
