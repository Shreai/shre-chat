    ...
    <div className="flex-1 flex flex-col h-full min-w-0">
      <header className="flex items-center justify-between px-4 py-3 shrink-0 backdrop-blur-sm"
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-semibold" style={{ color: "var(--c-text-1)" }}>{currentAgent.emoji} {currentAgent.name} Activity</h1>
          <span className="text-[10px]" style={{ color: "var(--c-text-5)" }}>{filtered.length} events</span>
        </div>
        {activity.length > 0 && (
          <button
            onClick={() => { localStorage.removeItem("shre-activity"); window.location.reload(); }}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        {grouped.size === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center gap-3 pb-20">
            <svg className="h-10 w-10" style={{ color: "var(--c-text-5)" }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12" /></svg>
            <p className="text-xs" style={{ color: "var(--c-text-4)" }}>No activity yet</p>
          </div>
        )}
              ...
              <div className="space-y-1 pl-3" style={{ borderLeft: "1px solid var(--c-border-2)" }}>
                {group.events.slice(0, 8).map((evt) => (
                  <div key={evt.id} className="flex items-center gap-2 py-0.5">
                    <span className={`text-xs ${STATUS_COLORS[evt.status]}`}>{STATUS_ICONS[evt.status]}</span>
                    <span className="text-[11px] flex-1" style={{ color: "var(--c-text-3)" }}>{evt.summary}</span>
                    <span className="text-[9px] shrink-0" style={{ color: "var(--c-text-5)" }}>{formatTime(evt.timestamp)}</span>
                  </div>
                ))}
                {group.events.length > 8 && (
                  <span className="text-[10px] pl-5" style={{ color: "var(--c-text-5)" }}>+{group.events.length - 8} more</span>
                )}
              </div>
            </div>
          ))}
