...
function renderMarkdown(text: string) {
            ...
            <tr>
              {headers.map((h, i) => (
                <th key={i} style={{
                  ...
                  borderBottom: "1px solid var(--c-border-2)", color: "var(--c-text-2)",
                  whiteSpace: "nowrap",
                }}>{formatInline(h.trim())}</th>
              ))}
            </tr>
          </thead>
          <tbody>
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.trim() === "") {
      ...
      elements.push(<p key={i} style={{ fontWeight: 700, color: "var(--c-text-1)", margin: "6px 0 2px", fontSize: "12px" }}>{formatInline(line)}</p>);
    } else if (/^\d+\.\s/.test(line)) {
      elements.push(<p key={i} style={{ paddingLeft: 12, color: "var(--c-text-3)", fontSize: "12px", margin: "2px 0" }}>{formatInline(line)}</p>);
    } else if (line.startsWith("- ")) {
      elements.push(<p key={i} style={{ paddingLeft: 12, color: "var(--c-text-3)", fontSize: "12px", margin: "2px 0" }}>{formatInline(line)}</p>);
    } else {
      elements.push(<p key={i} style={{ color: "var(--c-text-3)", fontSize: "12px", margin: "2px 0", lineHeight: 1.6 }}>{formatInline(line)}</p>);
    }
  }
...
function formatInline(text: string) {
  ...
  return parts.map((part, i) => {
    if (part.startsWith("**") && part.endsWith("**")) {
      return <strong key={i} style={{ color: "var(--c-text-1)", fontWeight: 600 }}>{part.slice(2, -2)}</strong>;
    }
...
// ── Demo Message Bubble ──────────────────────────────────────────────
function DemoBubble({ msg, agentName, agentEmoji }: { msg: DemoMessage; agentName: string; agentEmoji: string }) {
  const isUser = msg.role === "user";
  return (
    <div style={{ display: "flex", gap: 8, marginBottom: 12, justifyContent: isUser ? "flex-end" : "flex-start" }}>
      {!isUser && (
        <div style={{
          width: 28, height: 28, borderRadius: "50%", flexShrink: 0,
          background: "var(--c-bg-3)", display: "flex", alignItems: "center", justifyContent: "center",
          fontSize: 14,
        }}>{agentEmoji}</div>
      )}
      <div style={{
        ...
        {!isUser && (
          <div style={{ fontSize: 10, fontWeight: 600, color: "var(--c-text-4)", marginBottom: 4 }}>{agentName}</div>
        )}
...
export function DemoView() {
  ...
  function handleSuggestionClick(suggestion: string) {
    ...
    const match = DEMO_CONVERSATIONS.find(c =>
      c.messages[0]?.content.toLowerCase() === suggestion.toLowerCase()
    );
  ...
  return (
    <div style={{
      ...
      background: "var(--c-bg-1, #0a0a0f)", color: "var(--c-text-1, #e8e8f0)",
      ...
      <div style={{
        ...
        <a
          ...
          style={{
            ...
            background: "var(--c-accent, #6366f1)", color: "#fff", textDecoration: "none",
          }}
        >
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        ...
        <div style={{
          width: 240, flexShrink: 0, borderRight: "1px solid var(--c-border-1, #222233)",
          background: "var(--c-bg-2, #111118)", display: "flex", flexDirection: "column",
          overflow: "hidden",
        }}>
          <div style={{ padding: "14px 12px 8px", fontSize: 11, fontWeight: 600, color: "var(--c-text-4, #6a6a82)", textTransform: "uppercase", letterSpacing: "0.05em" }}>
          ...
          {DEMO_CONVERSATIONS.map(convo => (
            <button
              ...
              style={{
                ...
                borderLeft: activeConvo.id === convo.id ? "2px solid var(--c-accent, #6366f1)" : "2px solid transparent",
                transition: "all 0.15s",
              }}
            >
              <span style={{ fontSize: 16 }}>{convo.agentEmoji}</span>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{convo.title}</div>
                <div style={{ fontSize: 10, color: "var(--c-text-5, #555570)" }}>{convo.agentName}</div>
              </div>
            </button>
          ))}
        </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <div style={{
            ...
            borderBottom: "1px solid var(--c-border-1, #222233)", flexShrink: 0,
          }}>
            <span style={{ fontSize: 18 }}>{activeConvo.agentEmoji}</span>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--c-text-1)" }}>{activeConvo.agentName}</div>
            ...
            {typing && (
              <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                <div style={{
                  ...
                  background: "var(--c-bg-3)", display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 14,
                }}>{activeConvo.agentEmoji}</div>
                <div style={{
                  padding: "8px 12px", borderRadius: 12, background: "var(--c-bg-2)",
                  fontSize: 12, color: "var(--c-text-4)",
                }}>
          ...
          {!limitReached && demoResponses.length === 0 && (
            <div style={{
              ...
              <span style={{ fontSize: 10, color: "var(--c-text-5)", alignSelf: "center", marginRight: 4 }}>Try asking:</span>
              {DEMO_SUGGESTIONS.slice(0, 4).map(s => (
                <button
                  key={s}
                  onClick={() => handleSuggestionClick(s)}
                  style={{
                    padding: "4px 10px", borderRadius: 12, border: "1px solid var(--c-border-2, #2a2a3a)",
                    background: "var(--c-bg-2)", color: "var(--c-text-3)", fontSize: 11, cursor: "pointer",
                    transition: "all 0.15s",
                  }}
                  onMouseEnter={e => { (e.target as HTMLButtonElement).style.borderColor = "var(--c-accent, #6366f1)"; }}
                  onMouseLeave={e => { (e.target as HTMLButtonElement).style.borderColor = "var(--c-border-2, #2a2a3a)"; }}
                >
                  {s}
                </button>
              ))}
          ...
          {limitReached && (
            <div style={{
              ...
              <p style={{ fontSize: 14, fontWeight: 600, color: "var(--c-text-1)", marginBottom: 6 }}>
              ...
              <a
                ...
                style={{
                  ...
                  background: "var(--c-accent, #6366f1)", color: "#fff",
          ...
          {!limitReached && (
            <div style={{
              ...
              <textarea
                ...
                rows={1}
                style={{
                  flex: 1, resize: "none", border: "1px solid var(--c-border-2, #2a2a3a)",
                  borderRadius: 12, padding: "8px 12px", fontSize: 13,
                  background: "var(--c-bg-1)", color: "var(--c-text-1)",
