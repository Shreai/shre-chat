...
const SEVERITY_COLORS: Record<string, string> = {
  critical: "#f87171", warning: "#f59e0b", info: "#60a5fa", success: "#4ade80",
};
...
export function FeedAnalyticsView() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      setError(null);
      const result = await fetchApi<FeedAnalytics>(`/api/feed/analytics?since=${range.from}&until=${range.to}`);
      if (cancelled) return;
  ...
  const agentChart: ChartData | null = data?.byAgent.length ? {
    ...
    datasets: [{ label: "Events", data: data.byAgent.slice(0, 10).map(a => a.count), color: "#a78bfa" }],
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden" style={{ background: "var(--c-bg-1)" }}>
      <div className="px-4 py-3 flex items-center gap-2 justify-between" style={{ borderBottom: "1px solid var(--c-border-1)" }}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold" style={{ color: "var(--c-text-1)" }}>Feed Analytics</span>
          <span className="text-[10px] font-mono" style={{ color: "var(--c-text-5)" }}>via shre-feed</span>
        </div>
        <div className="flex items-center gap-2">
      ...
      {/* Severity toggles */}
      <div className="px-4 py-2 flex items-center gap-2" style={{ borderBottom: "1px solid var(--c-border-2)" }}>
        <span className="text-[10px]" style={{ color: "var(--c-text-5)" }}>Severity:</span>
        {["critical", "warning", "info"].map(s => (
          <button
            ...
            style={{
              background: severityFilter.has(s) ? (SEVERITY_COLORS[s] || "#60a5fa") + "22" : "var(--c-bg-2)",
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {!loading && data && (
          <>
            {timelineChart && (
              <div>
                <h3 className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--c-text-5)" }}>Event Volume</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {categoryChart && (
                <div>
                  <h3 className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--c-text-5)" }}>By Category</h3>
                  <div className="rounded-lg p-3" style={{ background: "var(--c-bg-2)", border: "1px solid var(--c-border-2)" }}>
              ...
              {agentChart && (
                <div>
                  <h3 className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--c-text-5)" }}>Top Agents</h3>
                  <div className="rounded-lg p-3" style={{ background: "var(--c-bg-2)", border: "1px solid var(--c-border-2)" }}>
            ...
            {data.byAgent.length > 0 && (
              <div>
                <h3 className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--c-text-5)" }}>Agent Breakdown</h3>
                <div className="space-y-1">
                  {data.byAgent.slice(0, 15).map(a => (
                    ...
                    >
                      <span className="text-[11px]" style={{ color: "var(--c-text-2)" }}>{a.agent}</span>
                      <span className="text-[11px] font-bold" style={{ color: "var(--c-text-3)" }}>{a.count}</span>
                    </button>
                  ))}
                </div>
              </div>
