...
function SortIcon({ active, dir }: { active: boolean; dir: 'asc' | 'desc' }) {
  if (!active) {
    return (
      <svg
        className="inline-block ml-0.5 h-3 w-3 opacity-30"
        viewBox="0 0 16 16"
        fill="currentColor"
      >
...
export function EmployeeActivityView() {
  ...
  const loadAlerts = useCallback(async () => {
    ...
    const result = await fetchApi<AlertEvent[]>(
      `/api/employee-activity/alerts?since=${encodeURIComponent(since)}`,
    );
  ...
  const columns: {
    key: SortKey;
    label: string;
    fmt: (e: EmployeeActivityData['employees'][0]) => string;
    warn?: (e: EmployeeActivityData['employees'][0]) => boolean;
  }[] = [
                          ...
                          {columns.map((col) => (
                            <th
                              ...
                              style={{
                                ...
                                background:
                                  sortKey === col.key ? 'rgba(255,255,255,0.03)' : undefined,
                              }}
                            >
                              {col.label}
