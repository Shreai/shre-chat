...
async function getCredentials(tenant: string = 'party-liquor') {
  try {
    ...
    throw new Error(
      'Failed to load RapidRMS credentials — set RAPIDRMS_EMAIL and RAPIDRMS_PASSWORD env vars',
    );
  }
}
...
export async function fetchPeriodData(
  ...
  try {
    ...
    const topCategory =
      Object.entries(categoryTotals).sort((a, b) => b[1] - a[1])[0]?.[0] || 'Bourbon';
...
export function formatDataForChat(data: PeriodData): string {
  ...
  const trendText =
    summary.trendPercent > 0 ? `+${summary.trendPercent}%` : `${summary.trendPercent}%`;
...
export function compareDataPeriods(period1: PeriodData, period2: PeriodData): string {
  ...
  const transactionPct =
    p2.totalTransactions > 0 ? Math.round((transactionDiff / p2.totalTransactions) * 100) : 0;
