...
import { sendMessage as sendChatMessage } from './router-client';
// ── Extracted modules ──
import { stripMd, detectCmd, detectAgentSwitch, } from './voice/voice-utils';
...
export default function VoiceAssistant({ open, onClose, messages, agentName, agentEmoji, agentId, ttsVoice, ttsProvider, agents, onSwitchAgent, onVoiceTurn, routerMode, models, selectedModel, onSelectModel, onSetTtsProvider, }) {
    ...
    const voiceSessionIdRef = useRef(sessionStorage.getItem('shre-voice-session-id') || '');
        ...
        if (!restored && voiceSessionIdRef.current) {
            fetch(`/api/voice-turns/${voiceSessionIdRef.current}`, { signal: AbortSignal.timeout(3000) })
                ...
                if (data?.turns?.length) {
                    const serverTurns = data.turns.map((t) => ({
                        role: t.role,
                        text: t.content || '',
                    }));
    ...
    const whisperAbortRef = useRef(null);
    ...
    const speak = useCallback(createSpeak({
        ttsVoice,
        ttsProvider,
        activeRef,
        ttsAbortRef,
    ...
    // ── AI request ──
    const askAI = useCallback(async (prompt, signal) => {
        try {
            const voiceTurns = turnHistoryRef.current.slice(-10).map((t) => ({
                role: t.role,
            ...
            const history = [...recentChat, ...voiceTurns];
            return await new Promise((resolve) => {
                ...
                sendChatMessage(prompt, history, `You are ${agentName}, a voice assistant. Keep responses concise. When the user asks for data (sales, invoices, inventory, top items, etc.), use markdown tables and formatting — the UI renders them visually. For conversational responses, be natural and brief. Always prefer structured data presentation (tables, bullet points) for data-heavy answers.`, callbacks, signal, voiceSessionIdRef.current || undefined, undefined, undefined, routerMode).catch((err) => {
                    clearTimeout(timeoutId);
    ...
    const clarifyContextRef = useRef(null);
    // ── Voice command ──
    const tryVoiceCommand = useCallback(async (text) => {
        try {
        ...
        try {
            ...
            if (cmd === 'goodbye') {
                ...
                if (!response || !activeRef.current) {
                    ...
                    if (activeRef.current && phaseRef.current === 'thinking')
            ...
            if (activeRef.current) {
                dispatch({
                    type: 'ERROR',
                    message: `Voice error: ${err?.message || 'Unknown error'}. Tap to retry.`,
                });
            }
        }
        finally {
    ...
    const startListening = useCallback(async () => {
        ...
        const p = phaseRef.current;
        if (p === 'speaking' || p === 'thinking' || p === 'transcribing')
            return;
    ...
    useEffect(() => {
        if (!proactiveMode || !open || pendingNotifs.length === 0)
            return;
        ...
        const notifTurn = { role: 'assistant', text: `[${notif.source}] ${text}` };
    ...
    return (_jsxs("div", { className: "fixed inset-0 z-[200] flex flex-col", style: { background: 'linear-gradient(180deg, #0a1628 0%, #0d1f3c 50%, #0a1628 100%)' }, children: [_jsxs("div", { className: "flex items-center justify-between px-5 pt-5 pb-2", style: { paddingTop: 'calc(1.25rem + env(safe-area-inset-top, 0px))' }, children: [_jsxs("button", { className: "flex items-center gap-2 rounded-full px-3 py-1.5 active:scale-95 transition-transform", style: {
                            background: agents && agents.length > 1 ? 'rgba(255,255,255,0.08)' : 'transparent',
                        }, onClick: () => agents && agents.length > 1 && setAgentPickerOpen((v) => !v), "aria-label": "Switch agent", children: [_jsx("span", { className: "text-lg", children: agentEmoji }), _jsx("span", { className: "text-sm font-medium", style: { color: 'rgba(255,255,255,0.9)' }, children: agentName }), agents && agents.length > 1 && (_jsx("svg", { className: "h-3.5 w-3.5", style: {
                                    color: 'rgba(255,255,255,0.4)',
                                    transform: agentPickerOpen ? 'rotate(180deg)' : '',
                                }, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: _jsx("polyline", { points: "6 9 12 15 18 9" }) }))] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("button", { onClick: () => setSettingsOpen((v) => !v), className: "h-10 w-10 rounded-full flex items-center justify-center active:scale-95 transition-transform", style: {
                                ...
                                }, "aria-label": "Voice settings", title: "Model, agent & voice settings", children: _jsxs("svg", { className: "h-4 w-4", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [_jsx("circle", { cx: "12", cy: "12", r: "3" }), _jsx("path", { d: "M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z" })] }) }), _jsxs("button", { onClick: () => setProactiveMode((v) => !v), className: "h-10 px-3 rounded-full flex items-center gap-1.5 active:scale-95 transition-transform text-[11px] font-medium", style: {
                                ...
                                }, "aria-label": proactiveMode ? 'Disable proactive notifications' : 'Enable proactive notifications', children: [_jsxs("svg", { className: "h-3.5 w-3.5", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [_jsx("path", { d: "M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" }), _jsx("path", { d: "M13.73 21a2 2 0 0 1-3.46 0" })] }), proactiveMode ? 'Live' : 'Alerts', proactiveMode && notifConnected && (_jsx("span", { className: "w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" })), proactiveMode && pendingNotifs.length > 0 && (_jsx("span", { className: "ml-0.5 text-[9px] bg-red-500 text-white rounded-full px-1", children: pendingNotifs.length }))] }), _jsx("button", { onClick: onClose, className: "h-10 w-10 rounded-full flex items-center justify-center active:scale-95 transition-transform", style: { background: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.6)' }, "aria-label": "Close", children: _jsxs("svg", { className: "h-5 w-5", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", children: [_jsx("line", { x1: "18", y1: "6", x2: "6", y2: "18" }), _jsx("line", { x1: "6", y1: "6", x2: "18", y2: "18" })] }) })] })] }), agentPickerOpen && agents && agents.length > 1 && (_jsx("div", { className: "mx-5 mb-2 rounded-xl overflow-hidden", style: {
                    background: 'rgba(255,255,255,0.06)',
                    border: '1px solid rgba(255,255,255,0.08)',
                }, children: agents
                    .filter((a) => a.id !== agentId)
                    .slice(0, 8)
                    .map((a) => (_jsxs("button", { className: "w-full flex items-center gap-3 px-4 py-2.5 text-left active:bg-white/10 transition-colors", style: {
                        color: 'rgba(255,255,255,0.85)',
                        borderBottom: '1px solid rgba(255,255,255,0.04)',
                    }, onClick: () => {
                        setAgentPickerOpen(false);
                        onSwitchAgent?.(a.id);
                    }, children: [_jsx("span", { className: "text-base", children: a.emoji }), _jsx("span", { className: "text-sm", children: a.name })] }, a.id))) })), settingsOpen && (_jsxs("div", { className: "mx-5 mb-2 rounded-xl p-4 space-y-3 animate-fadeIn", style: {
                    background: 'rgba(255,255,255,0.06)',
                    border: '1px solid rgba(255,255,255,0.08)',
                }, children: [models && models.length > 0 && (_jsxs("div", { children: [_jsx("label", { className: "text-[10px] font-medium uppercase tracking-wider mb-1.5 block", style: { color: 'rgba(255,255,255,0.35)' }, children: "Model" }), _jsxs("div", { className: "flex flex-wrap gap-1.5", children: [_jsx("button", { onClick: () => onSelectModel?.(null), className: "px-3 py-1.5 rounded-full text-[11px] active:scale-95 transition-all", style: {
                                        ...
                                        }, children: "Auto" }), models.slice(0, 8).map((m) => (_jsxs("button", { onClick: () => onSelectModel?.(m.id), className: "px-3 py-1.5 rounded-full text-[11px] active:scale-95 transition-all", style: {
                                        ...
                                        }, children: [m.icon, " ", m.name] }, m.id)))] })] })), agents && agents.length > 1 && (_jsxs("div", { children: [_jsx("label", { className: "text-[10px] font-medium uppercase tracking-wider mb-1.5 block", style: { color: 'rgba(255,255,255,0.35)' }, children: "Agent" }), _jsx("div", { className: "flex flex-wrap gap-1.5", children: agents.slice(0, 8).map((a) => (_jsxs("button", { onClick: () => { onSwitchAgent?.(a.id); }, className: "px-3 py-1.5 rounded-full text-[11px] active:scale-95 transition-all", style: {
                                    ...
                                    }, children: [a.emoji, " ", a.name] }, a.id))) })] })), _jsxs("div", { children: [_jsx("label", { className: "text-[10px] font-medium uppercase tracking-wider mb-1.5 block", style: { color: 'rgba(255,255,255,0.35)' }, children: "Voice Engine" }), _jsx("div", { className: "flex flex-wrap gap-1.5", children: [
                                ...
                                ].map((v) => (_jsx("button", { onClick: () => onSetTtsProvider?.(v.id), className: "px-3 py-1.5 rounded-full text-[11px] active:scale-95 transition-all", style: {
                                    ...
                                    }, children: v.label }, v.id))) })] })] })), micPermissionDenied && (_jsxs("div", { className: "mx-5 mb-2 rounded-xl p-4 flex items-start gap-3", style: {
                    background: 'rgba(239, 68, 68, 0.1)',
                    border: '1px solid rgba(239, 68, 68, 0.3)',
                }, children: [_jsxs("svg", { className: "h-5 w-5 shrink-0 mt-0.5", viewBox: "0 0 24 24", fill: "none", stroke: "#ef4444", strokeWidth: "2", strokeLinecap: "round", children: [_jsx("path", { d: "M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z" }), _jsx("path", { d: "M19 10v2a7 7 0 0 1-14 0v-2" }), _jsx("line", { x1: "1", y1: "1", x2: "23", y2: "23" })] }), _jsxs("div", { children: [_jsx("p", { className: "text-sm font-medium", style: { color: 'rgba(255,255,255,0.9)' }, children: "Microphone access blocked" }), _jsx("p", { className: "text-xs mt-1", style: { color: 'rgba(255,255,255,0.5)' }, children: /iPad|iPhone|iPod/.test(navigator.userAgent) || (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1)
                                    ...
                                    : /Android/i.test(navigator.userAgent)
                                        ...
                                        : 'Click the lock icon in the address bar \u2192 Site settings \u2192 Microphone \u2192 Allow.' }), _jsx("button", { className: "mt-2 text-xs px-3 py-1 rounded-full active:scale-95 transition-transform", style: { background: 'rgba(239, 68, 68, 0.2)', color: '#ef4444' }, onClick: async () => {
                                    setMicPermissionDenied(false);
                                    const stream = await acquireMic();
                                    if (stream) {
                                        dispatch({ type: 'GREETING_DONE' });
                                    }
                                }, children: "Try again" })] })] })), _jsxs("div", { ref: scrollRef, className: "flex-1 overflow-y-auto px-5 py-3 space-y-3", style: { minHeight: 0 }, children: [turns.map((t, i) => (_jsx("div", { className: `flex ${t.role === 'user' ? 'justify-end' : 'justify-start'} animate-fadeIn`, children: _jsxs("div", { className: "max-w-[85%] rounded-2xl px-4 py-2.5 text-[14px] leading-relaxed", style: {
                            ...
                            }, children: [_jsx(VoiceTurnContent, { text: t.text, role: t.role }), t.mib007Link && (_jsx("button", { onClick: () => window.open(`${window.location.hostname !== 'localhost' ? 'https://app.nirtek.net' : 'https://localhost:5520'}${t.mib007Link}`, '_blank'), className: "block mt-1.5 text-[12px] opacity-70 hover:opacity-100 transition-opacity underline", style: { color: 'rgba(96, 165, 250, 0.9)' }, children: "View in MIB007 \u2192" }))] }) }, i))), (phase === 'thinking' || phase === 'transcribing') && (_jsx("div", { className: "flex justify-start animate-fadeIn", children: _jsx("div", { className: "rounded-2xl px-4 py-3 flex items-center gap-2", style: {
                                background: 'rgba(255,255,255,0.06)',
                                border: '1px solid rgba(255,255,255,0.04)',
                            }, children: _jsxs("div", { className: "flex gap-1", children: [_jsx("span", { className: "w-2 h-2 rounded-full bg-blue-400 animate-bounce", style: { animationDelay: '0ms' } }), _jsx("span", { className: "w-2 h-2 rounded-full bg-blue-400 animate-bounce", style: { animationDelay: '150ms' } }), _jsx("span", { className: "w-2 h-2 rounded-full bg-blue-400 animate-bounce", style: { animationDelay: '300ms' } })] }) }) }))] }), briefingPlaying && (_jsx("div", { className: "absolute inset-0 z-10 flex items-end justify-center pb-8 cursor-pointer", onClick: skipBriefing, role: "button", "aria-label": "Skip briefing", children: _jsx("span", { className: "text-[11px] px-4 py-2 rounded-full", style: { background: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.5)' }, children: "Tap anywhere to skip briefing" }) })), shortcuts.length > 0 && state.phase === 'ready' && (_jsx("div", { className: "px-5 py-1.5 flex flex-wrap gap-2 justify-center", children: shortcuts.map((s) => (_jsx("button", { className: "text-[11px] px-3 py-1.5 rounded-full active:scale-95 transition-transform", style: {
                        background: 'rgba(255,255,255,0.06)',
                        color: 'rgba(255,255,255,0.6)',
                        border: '1px solid rgba(255,255,255,0.08)',
                    }, onClick: () => processUserInput(s.pattern), children: s.pattern.length > 30 ? s.pattern.slice(0, 28) + '...' : s.pattern }, s.id))) })), _jsx("div", { className: "px-5 min-h-[3rem] flex items-center justify-center", children: transcript && phase === 'listening' ? (_jsxs("p", { className: "text-sm text-center max-w-md leading-relaxed", style: { color: 'rgba(255,255,255,0.55)' }, children: ["\u201C", transcript, "\u201D"] })) : phase === 'error' ? (_jsx("p", { className: "text-xs text-center text-red-400 max-w-sm", children: errorMsg })) : null }), _jsxs("div", { className: "flex flex-col items-center pb-6", style: { paddingBottom: 'calc(1.5rem + env(safe-area-inset-bottom, 0px))' }, children: [_jsxs("button", { onClick: handleOrbTap, className: "relative h-24 w-24 rounded-full flex items-center justify-center active:scale-90", style: {
                                ...
                                : phase === 'transcribing'
                                    ...
                                    : phase === 'speaking'
                                        ...
                                        : phase === 'thinking'
                                            ...
                                            : phase === 'error'
                                                ? 'Tap to retry'
                                                : '', children: [phase === 'ready' ? (_jsxs("svg", { className: "h-10 w-10 text-white/80 drop-shadow", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "1.5", children: [_jsx("path", { d: "M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z" }), _jsx("path", { d: "M19 10v2a7 7 0 0 1-14 0v-2" }), _jsx("line", { x1: "12", y1: "19", x2: "12", y2: "23" }), _jsx("line", { x1: "8", y1: "23", x2: "16", y2: "23" })] })) : phase === 'listening' ? (_jsx("svg", { className: "h-9 w-9 text-white drop-shadow", viewBox: "0 0 24 24", fill: "currentColor", children: _jsx("rect", { x: "6", y: "6", width: "12", height: "12", rx: "2" }) })) : phase === 'speaking' ? (_jsxs("svg", { className: "h-9 w-9 text-white drop-shadow", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "1.5", children: [_jsx("polygon", { points: "11 5 6 9 2 9 2 15 6 15 11 19 11 5", fill: "white" }), _jsx("path", { d: "M15.54 8.46a5 5 0 0 1 0 7.07" }), _jsx("path", { d: "M19.07 4.93a10 10 0 0 1 0 14.14" })] })) : phase === 'error' ? (_jsxs("svg", { className: "h-9 w-9 text-white", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "1.5", children: [_jsx("circle", { cx: "12", cy: "12", r: "10" }), _jsx("line", { x1: "12", y1: "8", x2: "12", y2: "12" }), _jsx("line", { x1: "12", y1: "16", x2: "12.01", y2: "16" })] })) : (_jsxs("div", { className: "flex gap-1.5", children: [_jsx("span", { className: "w-2.5 h-2.5 rounded-full bg-white animate-bounce", style: { animationDelay: '0ms' } }), _jsx("span", { className: "w-2.5 h-2.5 rounded-full bg-white animate-bounce", style: { animationDelay: '150ms' } }), _jsx("span", { className: "w-2.5 h-2.5 rounded-full bg-white animate-bounce", style: { animationDelay: '300ms' } })] })), phase === 'listening' && (_jsx("span", { className: "absolute inset-0 rounded-full pointer-events-none", style: {
                                    border: `2px solid rgba(255,255,255,${speechActive ? 0.5 : 0.15})`,
                                    transform: `scale(${speechActive ? 1.2 : 1.05})`,
                                    transition: 'transform 150ms ease-out, border-color 150ms ease-out',
                                } }))] }), _jsx("span", { className: "text-[11px] mt-3 font-medium tracking-wide", style: { color: 'rgba(255,255,255,0.4)' }, children: phaseLabel }), phase === 'ready' && (_jsx("p", { className: "text-[10px] mt-1.5 text-center max-w-[220px] leading-relaxed", style: { color: 'rgba(255,255,255,0.35)' }, children: "Tap to talk" })), phase === 'listening' && (_jsx("p", { className: "text-[10px] mt-1.5 text-center max-w-[220px] leading-relaxed", style: { color: 'rgba(255,255,255,0.25)' }, children: "Tap to send" })), phase === 'transcribing' && (_jsx("p", { className: "text-[10px] mt-1.5", style: { color: 'rgba(255,255,255,0.25)' }, children: "Tap to cancel" })), phase === 'speaking' && (_jsx("p", { className: "text-[10px] mt-1.5", style: { color: 'rgba(255,255,255,0.25)' }, children: "Tap to interrupt" })), phase === 'thinking' && (_jsx("p", { className: "text-[10px] mt-1.5", style: { color: 'rgba(255,255,255,0.25)' }, children: "Tap to cancel" })), phase === 'error' && (_jsx("p", { className: "text-[10px] mt-1.5", style: { color: 'rgba(255,255,255,0.25)' }, children: "Tap to retry" }))] }), _jsx("style", { children: `
