    ...
    return new Promise((resolve, reject) => {
        ...
        req.onupgradeneeded = () => {
            ...
            if (!db.objectStoreNames.contains(STORE_ACTIVITY))
                db.createObjectStore(STORE_ACTIVITY);
            if (!db.objectStoreNames.contains(STORE_FEED))
                db.createObjectStore(STORE_FEED);
...
export async function idbLoadSessions() {
    return (await idbGet(STORE_SESSIONS, 'data')) ?? [];
}
export async function idbSaveSessions(sessions) {
    await idbPut(STORE_SESSIONS, 'data', sessions);
}
export async function idbLoadActivity() {
    return (await idbGet(STORE_ACTIVITY, 'data')) ?? [];
}
export async function idbSaveActivity(events) {
    await idbPut(STORE_ACTIVITY, 'data', events);
}
export async function idbLoadFeed() {
    return (await idbGet(STORE_FEED, 'data')) ?? [];
}
export async function idbSaveFeed(entries) {
    await idbPut(STORE_FEED, 'data', entries);
}
export async function idbLoadFiles() {
    return (await idbGet(STORE_FILES, 'data')) ?? [];
}
export async function idbSaveFiles(files) {
    await idbPut(STORE_FILES, 'data', files);
}
export async function idbLoadQueue() {
    return (await idbGet(STORE_QUEUE, 'data')) ?? [];
}
export async function idbSaveQueue(queue) {
    await idbPut(STORE_QUEUE, 'data', queue);
}
export async function idbLoadTabs() {
    return (await idbGet(STORE_META, 'tabs')) ?? [];
}
export async function idbSaveTabs(tabs) {
    await idbPut(STORE_META, 'tabs', tabs);
}
export async function idbLoadActiveSession() {
    return (await idbGet(STORE_META, 'activeSession')) ?? null;
}
export async function idbSaveActiveSession(id) {
    await idbPut(STORE_META, 'activeSession', id);
}
export async function idbLoadScrollPositions() {
    return (await idbGet(STORE_META, 'scrollPositions')) ?? {};
}
export async function idbSaveScrollPositions(positions) {
...
async function migrateFromLocalStorage() {
    ...
    const migrateStore = (storeName, key, lsKey) => {
        return new Promise((resolve, reject) => {
    ...
    await Promise.all([
        ...
        (async () => {
            ...
            if (active)
                await idbPut(STORE_META, 'activeSession', active);
        })(),
    ]);
