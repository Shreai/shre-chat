...
export function playNotifSound() {
    if (!isNotifEnabled() || document.hasFocus())
        return; // only play when tabbed away
    try {
...
export function playVoiceCue(type) {
    try {
...
export function showDesktopNotification(body) {
...
export function getTemplatesForAgent(agentId) {
...
export function getContextColor(pct) {
    if (pct > 90)
        return 'var(--c-danger)'; // red
    if (pct > 75)
        return 'var(--c-orange)'; // orange
    if (pct > 50)
        return 'var(--c-yellow)'; // yellow
    return 'var(--c-success)'; // green
}
export const MODEL_STORAGE_KEY = 'shre-model-overrides';
export function getModelOverride(agentId) {
    return usePreferences.getState().modelOverrides[agentId] || null;
}
export function setModelOverride(agentId, modelId) {
...
export function mib007Link(view, params) {
 ...
 */
export const MARKETPLACE_EMBED_APPS = {
    erp: {
        id: 'centrix',
        name: 'Centrix',
        icon: 'X',
...
export function lightweightMarkdown(text) {
    let html = text
...
export function splitStableAndPending(text) {
...
export function classifySystemEvent(content) {
    ...
    if (t.includes('system:'))
        return { icon: '⚙', label: 'System event', color: 'var(--c-slate)' };
