...
export function CronView() {
    ...
    useEffect(() => {
        async function checkServices() {
            const checks = CRON_JOBS.filter((j) => j.port).map(async (j) => {
                try {
    ...
    for (const job of CRON_JOBS) {
        if (!grouped.has(job.category))
            grouped.set(job.category, []);
        grouped.get(job.category).push(job);
    }
    const runningCount = Object.values(serviceStatus).filter((s) => s === 'running').length;
    return (_jsxs("div", { className: "flex-1 flex flex-col h-full min-w-0", children: [_jsx("header", { className: "flex items-center justify-between px-4 py-3 shrink-0 backdrop-blur-sm", style: { background: 'var(--c-bg-glass)', borderBottom: '1px solid var(--c-border-1)' }, children: _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("button", { onClick: () => actions.setSidebarOpen(!state.sidebarOpen), style: { color: 'var(--c-text-4)' }, children: _jsxs("svg", { className: "h-4 w-4", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [_jsx("line", { x1: "3", y1: "6", x2: "21", y2: "6" }), _jsx("line", { x1: "3", y1: "12", x2: "21", y2: "12" }), _jsx("line", { x1: "3", y1: "18", x2: "21", y2: "18" })] }) }), _jsx("h1", { className: "text-sm font-semibold", style: { color: 'var(--c-text-1)' }, children: "Cron Jobs" }), _jsxs("span", { className: "text-[10px]", style: { color: 'var(--c-text-5)' }, children: [CRON_JOBS.length, " jobs"] }), !loading && (_jsxs("span", { className: "text-[10px] text-emerald-400/60", children: [runningCount, " services up"] }))] }) }), _jsx("div", { className: "flex-1 overflow-y-auto px-4 py-4", children: _jsx("div", { className: "space-y-6 max-w-3xl mx-auto", children: Array.from(grouped.entries()).map(([cat, jobs]) => {
                        const cfg = CATEGORY_LABELS[cat];
                        return (_jsxs("div", { children: [_jsxs("div", { className: "flex items-center gap-2 mb-2", children: [_jsx("span", { className: `text-xs font-semibold ${cfg.color}`, children: cfg.label }), _jsx("span", { className: "text-[10px]", style: { color: 'var(--c-text-5)' }, children: jobs.length })] }), _jsx("div", { className: "space-y-0.5", children: jobs.map((job) => {
                                        const status = serviceStatus[job.id] || 'unknown';
                                        return (_jsxs("div", { className: "flex items-center gap-3 px-3 py-2 rounded-lg transition-colors", style: { background: 'transparent' }, onMouseEnter: (e) => (e.currentTarget.style.background = 'var(--c-bg-hover)'), onMouseLeave: (e) => (e.currentTarget.style.background = 'transparent'), children: [job.category === 'always-on' ? (_jsx(SBadge, { variant: status === 'running'
                                                        ? 'success'
                                                        : status === 'stopped'
                                                            ? 'destructive'
                                                            : 'warning', className: "text-[8px] px-1.5 py-0 h-4 shrink-0", children: status === 'running' ? 'up' : status === 'stopped' ? 'down' : '?' })) : (_jsx("span", { className: "h-2 w-2 rounded-full shrink-0 bg-blue-500/30" })), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "text-xs font-medium", style: { color: 'var(--c-text-2)' }, children: job.name }), job.port && (_jsxs("span", { className: "text-[9px] font-mono", style: { color: 'var(--c-text-5)' }, children: [":", job.port] }))] }), _jsx("span", { className: "text-[10px]", style: { color: 'var(--c-text-4)' }, children: job.description })] }), _jsxs("div", { className: "text-right shrink-0", children: [_jsx("span", { className: "text-[10px] font-mono", style: { color: 'var(--c-text-3)' }, children: job.schedule }), _jsx("div", { className: "text-[9px]", style: { color: 'var(--c-text-5)' }, children: job.interval })] }), _jsx("span", { className: "text-[9px] font-mono shrink-0 w-24 text-right truncate hidden lg:block", style: { color: 'var(--c-text-5)' }, children: job.service })] }, job.id));
                                    }) })] }, cat));
                    }) }) })] }));
}
