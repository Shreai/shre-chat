...
function RoutingModeIndicator() {
  ...
  const config: Record<string, { label: string; color: string; title: string }> = {
    cli:     { label: "CLI",      color: "#a855f7", title: "Claude Code CLI — coding tasks execute locally" },
    openclaw:{ label: "OpenClaw", color: "#8b5cf6", title: "OpenClaw Gateway — full agent workspace" },
  ...
  return (
    <div
      ...
      style={{ gap: 4, padding: "1px 6px", borderRadius: 4, background: `${c.color}15`, cursor: "default" }}
      title={c.title}
    >
      <span style={{ width: 5, height: 5, borderRadius: "50%", background: c.color, boxShadow: `0 0 4px ${c.color}` }} />
      <span style={{ fontSize: 11, fontWeight: 600, color: c.color, letterSpacing: "0.02em" }}>{c.label}</span>
    </div>
  );
}
...
export function StatusBar() {
  ...
  const fetchLiveTasks = useCallback(async () => {
    ...
    try {
      const token = sessionStorage.getItem("shre-auth-token") || localStorage.getItem("shre-auth-token");
      const res = await fetch("/api/tasks?limit=50", { headers: token ? { Authorization: `Bearer ${token}` } : {} });
      if (res.ok) {
        ...
        const activeStatuses = new Set(["in_progress", "started", "working_on", "pending_review", "review_needed", "blocked", "roadblock"]);
        tasks.sort((a, b) => {
  ...
  const fetchLiveAgents = useCallback(async () => {
    ...
    try {
      const token = sessionStorage.getItem("shre-auth-token") || localStorage.getItem("shre-auth-token");
      const res = await fetch("/api/agents", { headers: token ? { Authorization: `Bearer ${token}` } : {} });
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchLiveServices = useCallback(async () => {
    ...
    try {
      const token = sessionStorage.getItem("shre-auth-token") || localStorage.getItem("shre-auth-token");
      const res = await fetch("/api/platform-status", { headers: token ? { Authorization: `Bearer ${token}` } : {} });
      if (res.ok) {
        const data = await res.json();
  ...
  const taskAction = useCallback(async (taskId: string, action: "cancel" | "escalate", e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const reassignTask = useCallback(async (taskId: string, agentId: string) => {
    setTaskActionPending(taskId);
    try {
  ...
  const restartService = useCallback(async (serviceName: string, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    function connect() {
      ...
      ws.onmessage = (e) => {
        try {
          ...
          if (msg.type?.startsWith("task.") || msg.type?.startsWith("service.") || msg.type?.startsWith("agent.")) {
            fetchNotifications();
            fetchUnreadCount();
          }
  ...
  }, [bellOpen, notifFilter, fetchLiveTasks, fetchLiveAgents, fetchLiveServices, fetchNotifications, fetchUnreadCount]);
          ...
          >
            ...
            <div style={{
              ...
              <div style={{ display: "flex", gap: 4 }}>
                {NOTIF_FILTERS.map(f => {
                  const count = f.key === "tasks" ? liveTasks.length
                    : f.key === "agents" ? liveAgents.length
                    : f.key === "services" ? liveServices.length
                    : notifications.length;
                  const active = notifFilter === f.key;
                  return (
                    <button
                      key={f.key}
                      onClick={() => { setNotifFilter(f.key); setPanelSearch(""); }}
                      style={{
            ...
            <div style={{ overflowY: "auto", flex: 1 }}>
              {notifFilter === "tasks" ? (
                ...
                <>
                  {/* Search + refresh bar */}
                  <div style={{ padding: "6px 12px", borderBottom: "1px solid var(--c-border-2)", display: "flex", flexDirection: "column", gap: 4 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <div style={{ flex: 1, position: "relative" }}>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--c-text-4)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)" }}>
                        ...
                        <input
                          ...
                          style={{
                            width: "100%", boxSizing: "border-box", padding: "5px 8px 5px 26px", fontSize: 12,
                            background: "var(--c-bg-1)", border: "1px solid var(--c-border-2)", borderRadius: 6,
                            color: "var(--c-text-1)", outline: "none",
                          }}
                        />
                      </div>
                      <button onClick={fetchLiveTasks} disabled={liveTasksLoading}
                        style={{ background: "none", border: "none", cursor: "pointer", color: "var(--c-text-3)", padding: "4px", borderRadius: 4, flexShrink: 0, opacity: liveTasksLoading ? 0.5 : 1 }}
                        title="Refresh"
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ animation: liveTasksLoading ? "spin 1s linear infinite" : "none" }}>
                          <path d="M23 4v6h-6" /><path d="M1 20v-6h6" /><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15" />
                        </svg>
                      </button>
                    </div>
                    <div style={{ fontSize: 10, color: "var(--c-text-4)" }}>
                      {liveTasks.length} task{liveTasks.length !== 1 ? "s" : ""}{panelSearch ? ` · filtered` : ""} · auto-refreshes
                    </div>
                  </div>
                  {liveTasksLoading && liveTasks.length === 0 ? (
                    ...
                    TASK_GROUPS.map(group => {
                      ...
                      const groupTasks = liveTasks.filter(t => group.statuses.has(t.status) && (!panelSearch || t.title.toLowerCase().includes(searchLower) || t.agent?.toLowerCase().includes(searchLower) || t.status.includes(searchLower)));
                      if (groupTasks.length === 0) return null;
                      return (
                        <div key={group.label}>
                          {/* Group header */}
                          <div style={{
                            padding: "6px 16px", fontSize: 10, fontWeight: 700, letterSpacing: "0.05em",
                            textTransform: "uppercase", color: "var(--c-text-4)",
                            background: "var(--c-bg-1)", borderBottom: "1px solid var(--c-border-2)",
                            position: "sticky", top: 0, zIndex: 1,
                          }}>
                            {group.label} ({groupTasks.length})
                          </div>
                          {groupTasks.map(task => {
                            const cfg = TASK_STATUS_CONFIG[task.status] || { color: "#6b7280", label: task.status, icon: "\u25cb" };
                            const isActive = ["in_progress", "started", "working_on"].includes(task.status);
                            const isPending = taskActionPending === task.id;
                            const isTerminal = ["done", "completed", "cancelled", "qa_tested", "production_ready"].includes(task.status);
                            return (
                              <div
                                key={task.id}
                              ...
                              >
                                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                                  ...
                                  {task.priority && ["high", "critical"].includes(task.priority) && (
                                    <span style={{ fontSize: 10, fontWeight: 600, color: task.priority === "critical" ? "#ef4444" : "#f59e0b" }}>
                                  ...
                                  {!isTerminal && (
                                    <button
                                      onClick={(e) => { e.stopPropagation(); setTaskActionMenu(taskActionMenu === task.id ? null : task.id); setShowAssignDropdown(null); }}
                                      style={{
                                        marginLeft: "auto", background: "none", border: "none", cursor: "pointer",
                                        color: "var(--c-text-4)", padding: "2px 4px", borderRadius: 4, fontSize: 14, lineHeight: 1,
                                      }}
                                      title="Actions"
                                    >
                                      &#x22EE;
                                ...
                                {taskActionMenu === task.id && (
                                  <div style={{
                                    ...
                                    <button onClick={(e) => taskAction(task.id, "escalate", e)} style={{ width: "100%", textAlign: "left", background: "none", border: "none", cursor: "pointer", padding: "6px 10px", borderRadius: 4, fontSize: 12, color: "#f59e0b" }}
                                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "var(--c-bg-hover)"; }}
                                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "none"; }}
                                    >
                                      Escalate to critical
                                    </button>
                                    <button onClick={(e) => { e.stopPropagation(); setShowAssignDropdown(showAssignDropdown === task.id ? null : task.id); }} style={{ width: "100%", textAlign: "left", background: "none", border: "none", cursor: "pointer", padding: "6px 10px", borderRadius: 4, fontSize: 12, color: "var(--c-text-2)" }}
                                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "var(--c-bg-hover)"; }}
                                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "none"; }}
                                    >
                                      Reassign agent
                                    ...
                                    {showAssignDropdown === task.id && liveAgents.length > 0 && (
                                      <div style={{ padding: "4px 0", borderTop: "1px solid var(--c-border-2)", marginTop: 2 }}>
                                        {liveAgents.filter(a => a.id !== task.agent).slice(0, 8).map(a => (
                                          <button key={a.id} onClick={(e) => { e.stopPropagation(); reassignTask(task.id, a.id); }}
                                            style={{ width: "100%", textAlign: "left", background: "none", border: "none", cursor: "pointer", padding: "4px 10px 4px 18px", borderRadius: 4, fontSize: 11, color: "var(--c-text-2)" }}
                                            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "var(--c-bg-hover)"; }}
                                            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "none"; }}
                                          >
                                            {a.name}
                                    ...
                                    <div style={{ borderTop: "1px solid var(--c-border-2)", marginTop: 2, paddingTop: 2 }}>
                                      <button onClick={(e) => taskAction(task.id, "cancel", e)} style={{ width: "100%", textAlign: "left", background: "none", border: "none", cursor: "pointer", padding: "6px 10px", borderRadius: 4, fontSize: 12, color: "#ef4444" }}
                                        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "var(--c-bg-hover)"; }}
                                        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "none"; }}
                                      >
                                        Cancel task
                                ...
                                <div style={{ fontSize: 13, fontWeight: 500, color: "var(--c-text-1)", lineHeight: 1.4, marginBottom: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                  {task.title}
                                </div>
                                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                                  {task.agent && (
                                    <span style={{ fontSize: 11, color: "var(--c-text-2)", display: "flex", alignItems: "center", gap: 3 }}>
                                  ...
                                  {task.quality_score != null && (
                                    <span style={{ fontSize: 11, color: task.quality_score >= 0.8 ? "#22c55e" : task.quality_score >= 0.5 ? "#f59e0b" : "#ef4444" }}>
                                      Q: {(task.quality_score * 100).toFixed(0)}%
                                    </span>
                                  )}
                                  {task.completion_ratio != null && task.completion_ratio > 0 && task.completion_ratio < 1 && (
                                    <span style={{ fontSize: 11, color: "var(--c-text-3)" }}>
                                      {(task.completion_ratio * 100).toFixed(0)}% done
                                    </span>
                                  )}
                                  <span style={{ fontSize: 10, color: "var(--c-text-4)", marginLeft: "auto" }}>
                                    {new Date(task.updated_at || task.created_at).toLocaleString([], { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}
                                  </span>
                                </div>
                              </div>
                            );
                ...
                <>
                  <div style={{ padding: "6px 12px", borderBottom: "1px solid var(--c-border-2)", display: "flex", flexDirection: "column", gap: 4 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <div style={{ flex: 1, position: "relative" }}>
                        ...
                        <input type="text" placeholder="Search agents..." value={panelSearch} onChange={e => setPanelSearch(e.target.value)}
                          style={{ width: "100%", boxSizing: "border-box", padding: "5px 8px 5px 26px", fontSize: 12, background: "var(--c-bg-1)", border: "1px solid var(--c-border-2)", borderRadius: 6, color: "var(--c-text-1)", outline: "none" }}
                        />
                      </div>
                      <button onClick={fetchLiveAgents} disabled={liveAgentsLoading}
                        style={{ background: "none", border: "none", cursor: "pointer", color: "var(--c-text-3)", padding: "4px", borderRadius: 4, flexShrink: 0, opacity: liveAgentsLoading ? 0.5 : 1 }}
                        title="Refresh"
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ animation: liveAgentsLoading ? "spin 1s linear infinite" : "none" }}>
                          <path d="M23 4v6h-6" /><path d="M1 20v-6h6" /><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15" />
                        </svg>
                      </button>
                    </div>
                    <div style={{ fontSize: 10, color: "var(--c-text-4)" }}>
                      {liveAgents.filter(a => a.status === "busy").length} busy / {liveAgents.length} total
                    </div>
                  </div>
                  {liveAgentsLoading && liveAgents.length === 0 ? (
                    ...
                    liveAgents.filter(a => !panelSearch || a.name.toLowerCase().includes(panelSearch.toLowerCase()) || a.model.toLowerCase().includes(panelSearch.toLowerCase()) || a.currentTask?.title?.toLowerCase().includes(panelSearch.toLowerCase())).map(agent => {
                      const isBusy = agent.status === "busy";
                      const task = agent.currentTask;
                      return (
                        <div
                          key={agent.id}
                          style={{ padding: "10px 16px", borderBottom: "1px solid var(--c-border-2)", transition: "background 0.15s" }}
                          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "var(--c-bg-hover)"; }}
                          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "transparent"; }}
                        >
                          ...
                          {/* Current task info */}
                          {task && (
                            <div style={{ marginTop: 6, marginLeft: 16, padding: "4px 8px", borderRadius: 4, background: "var(--c-bg-1)", fontSize: 11, color: "var(--c-text-2)" }}>
                              <div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontWeight: 500 }}>
                                {task.title}
                              </div>
                              <div style={{ display: "flex", gap: 8, marginTop: 2, color: "var(--c-text-4)", fontSize: 10 }}>
                                {task.phase && <span>{task.phase}</span>}
                                {task.progress && <span>{task.progress}</span>}
                                {task.elapsedMs != null && <span>{Math.round(task.elapsedMs / 60000)}m elapsed</span>}
                                {task.type && <span style={{ marginLeft: "auto", opacity: 0.7 }}>{task.type}</span>}
                              </div>
                            </div>
                          )}
                        </div>
                ...
                <>
                  <div style={{ padding: "6px 12px", borderBottom: "1px solid var(--c-border-2)", display: "flex", flexDirection: "column", gap: 4 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <div style={{ flex: 1, position: "relative" }}>
                        ...
                        <input type="text" placeholder="Search services..." value={panelSearch} onChange={e => setPanelSearch(e.target.value)}
                          style={{ width: "100%", boxSizing: "border-box", padding: "5px 8px 5px 26px", fontSize: 12, background: "var(--c-bg-1)", border: "1px solid var(--c-border-2)", borderRadius: 6, color: "var(--c-text-1)", outline: "none" }}
                        />
                      </div>
                      <button onClick={fetchLiveServices} disabled={liveServicesLoading}
                        style={{ background: "none", border: "none", cursor: "pointer", color: "var(--c-text-3)", padding: "4px", borderRadius: 4, flexShrink: 0, opacity: liveServicesLoading ? 0.5 : 1 }}
                        title="Refresh"
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ animation: liveServicesLoading ? "spin 1s linear infinite" : "none" }}>
                          <path d="M23 4v6h-6" /><path d="M1 20v-6h6" /><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15" />
                        </svg>
                      </button>
                    </div>
                  ...
                  {liveServicesLoading && liveServices.length === 0 ? (
                    ...
                    <div style={{ padding: "48px 16px", textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: 13, color: "var(--c-text-3)" }}>No service data available</div>
                    </div>
                  ) : (
                    liveServices.filter(s => !panelSearch || s.name.toLowerCase().includes(panelSearch.toLowerCase()) || s.status?.toLowerCase().includes(panelSearch.toLowerCase())).map(svc => {
                        ...
                        >
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontSize: 13, fontWeight: 500, color: "var(--c-text-1)" }}>
                                {svc.name}
                                {svc.port && <span style={{ fontSize: 10, color: "var(--c-text-4)", marginLeft: 4 }}>:{svc.port}</span>}
                              </div>
                              <div style={{ display: "flex", gap: 8, fontSize: 10, color: "var(--c-text-4)", marginTop: 2 }}>
                                {svc.latency_ms != null && (
                                  <span style={{ color: svc.latency_ms < 100 ? "#22c55e" : svc.latency_ms < 500 ? "#f59e0b" : "#ef4444" }}>
                                    {svc.latency_ms}ms
                                  </span>
                                )}
                                {svc.uptime_pct != null && (
                                  <span style={{ color: svc.uptime_pct >= 99.9 ? "#22c55e" : svc.uptime_pct >= 95 ? "#f59e0b" : "#ef4444" }}>
                              ...
                              {!svc.healthy && (
                                <button
                                  ...
                                  style={{
                                    ...
                                    color: "#ef4444", cursor: isRestarting ? "wait" : "pointer",
                                ...
                                >
                                  {isRestarting ? (
                                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ animation: "spin 1s linear infinite" }}>
                                      <path d="M23 4v6h-6" /><path d="M1 20v-6h6" /><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15" />
                                    </svg>
                                  ) : (
                                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                <>
                  ...
                  <div style={{ padding: "6px 12px", borderBottom: "1px solid var(--c-border-2)" }}>
                    <div style={{ position: "relative" }}>
                      ...
                      <input type="text" placeholder="Search notifications..." value={panelSearch} onChange={e => setPanelSearch(e.target.value)}
                        style={{ width: "100%", boxSizing: "border-box", padding: "5px 8px 5px 26px", fontSize: 12, background: "var(--c-bg-1)", border: "1px solid var(--c-border-2)", borderRadius: 6, color: "var(--c-text-1)", outline: "none" }}
                      />
                    </div>
                  </div>
                  ...
                  {(() => {
                    ...
                    const hasLiveData = activeTasks.length > 0 || blockedTasks.length > 0 || unhealthySvcs.length > 0 || busyAgents.length > 0;
                    if (!hasLiveData) return null;
                    return (
                      <div style={{ padding: "8px 12px", borderBottom: "1px solid var(--c-border-2)", display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {activeTasks.length > 0 && (
                          <button onClick={() => setNotifFilter("tasks")} style={{ background: "#8b5cf615", border: "1px solid #8b5cf630", borderRadius: 6, padding: "4px 8px", fontSize: 11, color: "#8b5cf6", cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
                            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#8b5cf6", animation: "pulse-dot 2s ease-in-out infinite" }} />
                            {activeTasks.length} active task{activeTasks.length !== 1 ? "s" : ""}
                          </button>
                        )}
                        {blockedTasks.length > 0 && (
                          <button onClick={() => setNotifFilter("tasks")} style={{ background: "#ef444415", border: "1px solid #ef444430", borderRadius: 6, padding: "4px 8px", fontSize: 11, color: "#ef4444", cursor: "pointer" }}>
                            {blockedTasks.length} blocked
                          </button>
                        )}
                        {busyAgents.length > 0 && (
                          <button onClick={() => setNotifFilter("agents")} style={{ background: "#8b5cf615", border: "1px solid #8b5cf630", borderRadius: 6, padding: "4px 8px", fontSize: 11, color: "#8b5cf6", cursor: "pointer" }}>
                            {busyAgents.length} agent{busyAgents.length !== 1 ? "s" : ""} busy
                          </button>
                        )}
                        {unhealthySvcs.length > 0 && (
                          <button onClick={() => setNotifFilter("services")} style={{ background: "#ef444415", border: "1px solid #ef444430", borderRadius: 6, padding: "4px 8px", fontSize: 11, color: "#ef4444", cursor: "pointer" }}>
                  ...
                  {(() => {
                    const filtered = panelSearch
                      ? notifications.filter(n => n.title.toLowerCase().includes(panelSearch.toLowerCase()) || n.body?.toLowerCase().includes(panelSearch.toLowerCase()) || n.source?.toLowerCase().includes(panelSearch.toLowerCase()))
                      : notifications;
                    ...
                    <div style={{ padding: "48px 16px", textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: 13, color: "var(--c-text-3)" }}>{panelSearch ? "No matching notifications" : "No notifications yet"}</div>
                    </div>
                  ) : (
                    filtered.map(n => (
                      <div
