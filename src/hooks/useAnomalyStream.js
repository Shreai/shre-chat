...
const RAPIDRMS_SSE_BASE = typeof window !== 'undefined'
    ? window.__RAPIDRMS_URL__ ||
        import.meta.env?.VITE_RAPIDRMS_URL ||
        'http://localhost:8899'
    : 'http://localhost:8899';
/**
 ...
 */
async function discoverWorkspaceId(baseUrl) {
    try {
...
export function useAnomalyStream({ workspaceId: workspaceIdProp, baseUrl = RAPIDRMS_SSE_BASE, onCritical, } = {}) {
    const [anomalies, setAnomalies] = useState([]);
    const [connected, setConnected] = useState(false);
    const [dismissed, setDismissed] = useState(false);
    // Resolved workspace: prop > localStorage > discovered from session endpoint
    const [workspaceId, setWorkspaceId] = useState(workspaceIdProp ?? localStorage.getItem('rapidrms-workspace'));
    ...
    const connect = useCallback(() => {
        if (!workspaceId || !mountedRef.current)
            return;
