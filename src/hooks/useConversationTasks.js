 ...
 */
export function useConversationTasks(sessionId) {
    ...
    const fetchTasks = useCallback(async () => {
        if (!sessionId)
            return;
        try {
            const res = await fetch(`/api/tasks?session_id=${encodeURIComponent(sessionId)}&source=conversation-loop&limit=20`, { signal: AbortSignal.timeout(5000) });
            if (!res.ok)
                return;
