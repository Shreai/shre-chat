...
import { setAgent, fetchAllAgentMessages, fetchAvailableModels, } from '../router-client';
...
import { loadScrollPositions, saveScrollPositions } from '../store';
import { showDesktopNotification, getModelOverride, setModelOverride } from '../chat-utils';
export function useChatEffects(params) {
    const { activeSessionId, activeAgentId, streaming, streamText, sessions, messages, filteredMessages, actions, scrollRef, inputRef, streamFlushRaf, streamBufferRef, sendingRef, abortRef, setInput, setSelectedModel, setDynamicModels, setRouterUp, setCompareModels, showEmoji, setShowEmoji, emojiRef, showModelPicker, setShowModelPicker, modelPickerRef, comparePickerOpen, setComparePickerOpen, comparePickerRef, setShareUrl, setSharedSnapshot, setSharedLoading, setSharedError, generateTitle, virtualizer, } = params;
    const scrollPositionsRef = useRef(loadScrollPositions());
    const scrollSaveTimerRef = useRef(null);
    const prevSessionIdRef = useRef(null);
    const prevMsgCount = useRef(0);
    const newMsgStartIndex = useRef(null);
    const initialLoadDone = useRef(false);
    const [showJumpToLatest, setShowJumpToLatest] = useState(false);
    const userNearBottomRef = useRef(true);
    const showJumpRef = useRef(false);
    const jumpDebounceRef = useRef(null);
    ...
    const sentHistoryRef = useRef(initHistory);
    ...
    useEffect(() => {
        ...
        async function syncFromRouter() {
            ...
            try {
                ...
                const syncTimeout = new Promise((_, reject) => setTimeout(() => reject(new Error('sync-timeout')), 8000));
                ...
                const rawMsgs = result.messages.filter((m) => (m.role === 'user' || m.role === 'assistant') &&
                    m.content?.trim() &&
                ...
                try {
                    ...
                    if (stored)
                        liveSessions = JSON.parse(stored);
                }
                catch (_) {
                    void _;
                }
                const existingSession = liveSessions.find((s) => s.id === activeSessionId) ||
                    liveSessions.find((s) => (s.agentId || 'main') === activeAgentId);
                if (existingSession) {
                    if (isInitialSync) {
                        ...
                        const routerNew = chatMessages.filter((m) => m.timestamp &&
                        ...
                        const newMsgs = chatMessages.filter((m) => m.role === 'assistant' &&
                            !localContents.has(`${m.role}:${m.content.trim().slice(0, 120)}`));
                        for (const msg of newMsgs) {
                            actions.addMessage(existingSession.id, msg);
                            actions.addFeed(existingSession.id, 'received', `[Router] ${msg.content.slice(0, 80)}${msg.content.length > 80 ? '\u2026' : ''}`, { source: 'router', agent: activeAgentId, model: msg.model || '' });
                        }
                    }
                }
                else {
        ...
        const iv = setInterval(async () => {
            if (streaming || isWSConnected() || recentWSSendRef.current)
                return;
    ...
    const handleScroll = useCallback(() => {
        ...
        if (shouldShow !== showJumpRef.current) {
            ...
            if (jumpDebounceRef.current)
                clearTimeout(jumpDebounceRef.current);
    ...
    const handlePullMove = useCallback((e) => {
        if (!pullStartRef.current || pullRefreshing)
            return;
        const el = scrollRef.current;
    ...
    useEffect(() => {
        ...
        if (prevId && !isSameSession) {
            const el = scrollRef.current;
            if (el)
                scrollPositionsRef.current[prevId] = el.scrollTop;
    ...
    useEffect(() => {
        if (sentHistoryRef.current.length > 0)
            return;
