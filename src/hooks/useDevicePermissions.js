...
function persistPermissions(perms) {
    try {
...
export function useDevicePermissions() {
    ...
    useEffect(() => {
        const checkAll = async () => {
            const names = ['microphone', 'camera', 'notifications', 'location', 'clipboard-read'];
            const updates = {};
    ...
    const requestPermission = useCallback(async (name) => {
        if (!isAPISupported(name)) {
            setPermissions((prev) => {
                const next = { ...prev, [name]: { ...prev[name], status: 'unsupported', lastChecked: Date.now() } };
                persistPermissions(next);
                return next;
            });
            return false;
    ...
    const isPermanentlyDenied = useCallback((name) => {
    ...
    const getSettingsInstructions = useCallback((name) => {
