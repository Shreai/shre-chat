 ...
 */
export function anchorContextIfNeeded(text, sendText, replyToIndex, filteredMessages) {
 ...
 */
export async function fetchContextSources(sessionId) {
    let taskContext = '';
    let sessionContext = '';
    const contextHealth = {
        tasks: 'missing',
        ...
        if (taskResult.status === 'fulfilled' && taskResult.value.ok) {
            ...
            if (tasks.length > 0) {
                ...
                for (const [status, items] of Object.entries(byStatus)) {
                    const list = items
                        .slice(0, 20)
                        .map((t) => `  - ${t.title}${t.id ? ` (ID: ${t.id})` : ''}${t.priority ? ` [${t.priority}]` : ''}${t.assignee ? ` \u2192 ${t.assignee}` : ''}`)
                        .join('\n');
            ...
            if (ctxData.recentSessions?.length) {
                parts.push('Recent conversations:\n' +
                    ctxData.recentSessions
                        .map((s) => `  - [${new Date(s.updated_at).toLocaleDateString()}] "${s.title}" (${s.agent_id || 'shre'}, ${s.messageCount || '?'} messages)`)
                        .join('\n'));
            }
            if (ctxData.voiceSummaries?.length) {
                parts.push('Recent voice sessions:\n' +
                    ctxData.voiceSummaries
                        .map((v) => `  - [${new Date(v.created_at).toLocaleDateString()}] ${v.summary || 'No summary'} (${v.agent_id || 'shre'})`)
                        .join('\n'));
            }
            if (ctxData.recentActions?.length) {
                parts.push('Recent actions taken:\n' +
                    ctxData.recentActions
                        .map((a) => `  - [${new Date(a.created_at).toLocaleDateString()}] ${a.action_type}: ${a.target || a.result || ''}`)
                        .join('\n'));
            }
            if (parts.length) {
