...
export async function fetchContextSources(sessionId: string): Promise<{
  ...
  } catch (err) { console.debug("build task context", err); contextHealth.tasks = "error"; }
  ...
  } catch (err) { console.debug("build cross-session context", err); contextHealth.crossSession = "error"; }
