...
export function useMessageListHandlers({
  ...
  const onSelectTemplate = useCallback((prompt: string) => {
    setInput(prompt);
  ...
  const onEditStart = useCallback((msgIndex: number, content: string) => {
  ...
  const onEdit = useCallback((msgIndex: number, newText: string) => {
  ...
  }, [activeSessionId, messages, actions, setEditingMsgIndex, setEditingMsgText, setInput, pendingEditSendRef]);

  const onRegenerate = useCallback((_msgIndex: number) => {
    if (!activeSessionId) return;
  ...
  const onAnnotate = useCallback((msgIndex: number, text: string) => {
    if (activeSessionId) actions.setAnnotation(activeSessionId, msgIndex, text);
  }, [activeSessionId, actions]);

  const onBranch = useCallback((msgIndex: number) => {
    if (!activeSessionId) return;
  ...
  const onReaction = useCallback((msgIndex: number, emoji: string) => {
    if (activeSessionId) actions.toggleReaction(activeSessionId, msgIndex, emoji);
  }, [activeSessionId, actions]);

  const onReply = useCallback((msgIndex: number) => {
    actions.setReplyTo(msgIndex);
    inputRef.current?.focus();
  }, [actions, inputRef]);

  const onRetry = useCallback((msgIndex: number) => {
    if (!activeSessionId) return;
