...
export function useHeaderActions({ activeSessionId, activeSession, messages, userName, currentAgentName, currentAgentId, actions, }) {
    ...
    const handleSetGatewayMode = useCallback((mode) => setGatewayMode(mode), [setGatewayMode]);
    const handleToggleCompare = useCallback((compareModelsLength) => {
        if (!compareMode) {
            setCompareMode(true);
            if (compareModelsLength < 2)
    ...
    const handleCopyMarkdown = useCallback(() => {
        const md = messages
            .map((m) => `**${m.role === 'user' ? userName : currentAgentName}** (${formatTime(m.timestamp)}):\n${m.content}`)
            .join('\n\n---\n\n');
