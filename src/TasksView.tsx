...
export function TasksView() {
  ...
  const [statusFilter, setStatusFilter] = useState(
    'created,todo,in_progress,pending_review,blocked',
  );
  ...
  const filtered = tasks.filter((t) => {
    ...
    return (
      t.title.toLowerCase().includes(q) || t.agent?.toLowerCase().includes(q) || t.id.includes(q)
    );
  });
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={load}
            className="text-[11px] px-2 py-1 rounded-lg transition-colors hover:bg-white/5"
            style={{ color: 'var(--c-text-3)' }}
          >
            Refresh
          </button>
      ...
      >
        ...
        <div className="flex items-center gap-1 flex-wrap">
          <button
            ...
            style={{
              background:
                statusFilter.includes('created') && !statusFilter.includes('done')
                  ? 'var(--c-accent)'
                  : 'transparent',
              color:
                statusFilter.includes('created') && !statusFilter.includes('done')
        ...
        <div className="px-3 pb-3 space-y-2">
          ...
          >
            ...
            {task.completion_ratio != null && (
              <span>Progress: {(task.completion_ratio * 100).toFixed(0)}%</span>
            )}
