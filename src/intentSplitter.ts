...
const INTENT_LEAD_PATTERNS: Array<{ regex: RegExp; type: IntentType }> = [
  ...
  { regex: /^(?:please\s+)?(?:create|add|make)\s+(?:a\s+)?(?:task|to-?do|reminder)\b/i, type: "task" },
  { regex: /^todo[:\s]/i, type: "task" },
  { regex: /^task[:\s]/i, type: "task" },
  { regex: /^i\s+need\s+to\s+(?!know|understand|ask|find\s+out)/i, type: "task" },

  // Issue creation
  { regex: /^(?:please\s+)?(?:create|add|open|file|log|submit)\s+(?:an?\s+)?(?:issue|bug|ticket)\b/i, type: "issue" },
  ...
  { regex: /^(?:please\s+)?(?:fetch|get|show|pull|give|find|look\s+up|check|display|list|what|how|when|where|who|can\s+you)\b/i, type: "query" },
...
export function splitIntents(message: string): SplitResult {
  ...
  if (trimmed.length < 15 || !SPLIT_CONJUNCTIONS.test(trimmed)) {
    return { wasSplit: false, intents: [{ text: trimmed, type: classifySegment(trimmed), confidence: 1 }], original: trimmed };
  }
