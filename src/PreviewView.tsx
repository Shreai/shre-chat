...
function detectType(content: string, title?: string): PreviewType {
  ...
  // Sniff content
  if (content.trimStart().startsWith("<!DOCTYPE") || content.trimStart().startsWith("<html") || content.includes("</div>") || content.includes("</table>")) return "html";
...
function TxtRenderer({ content }: { content: string }) {
  return (
    <div className="w-full h-full overflow-auto" style={{ background: "#0d1117", padding: 24 }}>
      <pre style={{ color: "#e6edf3", fontSize: 13, fontFamily: "'JetBrains Mono', 'SF Mono', monospace", lineHeight: 1.6, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
        {content}
      </pre>
    </div>
  );
  ...
  return (
    <div className="w-full h-full overflow-auto" style={{ background: "#0d1117" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, fontFamily: "'JetBrains Mono', monospace" }}>
        <thead>
          <tr>
            <th style={{ position: "sticky", top: 0, padding: "8px 12px", background: "#161b22", color: "#58a6ff", borderBottom: "2px solid #30363d", textAlign: "left", fontWeight: 600, whiteSpace: "nowrap" }}>#</th>
            {headers.map((h, i) => (
              <th key={i} style={{ position: "sticky", top: 0, padding: "8px 12px", background: "#161b22", color: "#e6edf3", borderBottom: "2px solid #30363d", textAlign: "left", fontWeight: 600, whiteSpace: "nowrap" }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} style={{ background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.02)" }}>
              <td style={{ padding: "6px 12px", borderBottom: "1px solid #21262d", color: "#484f58", textAlign: "right" }}>{i + 1}</td>
              {headers.map((_, j) => (
                <td key={j} style={{ padding: "6px 12px", borderBottom: "1px solid #21262d", color: "#e6edf3", textAlign: isNum(row[j] || "") ? "right" : "left", whiteSpace: "nowrap" }}>{row[j] ?? ""}</td>
              ))}
            </tr>
          ))}
        </tbody>
      </table>
      <div style={{ padding: "8px 12px", color: "#484f58", fontSize: 11 }}>{rows.length} rows × {headers.length} columns</div>
    </div>
  );
}
...
function JsonRenderer({ content }: { content: string }) {
  ...
  return (
    <div className="w-full h-full overflow-auto" style={{ background: "#0d1117", padding: 24 }}>
      <pre style={{ color: "#e6edf3", fontSize: 12, fontFamily: "'JetBrains Mono', 'SF Mono', monospace", lineHeight: 1.5, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
        {formatted}
      </pre>
    </div>
  );
...
export function PreviewView() {
  ...
  const openInNewTab = () => {
    ...
    if (type === "html" || type === "markdown") {
      ...
      if (w) {
        w.document.write(type === "markdown" ? `<!DOCTYPE html><html><body>${markdownToSimpleHtml(active.html)}</body></html>` : active.html);
        w.document.close();
      }
    } else {
      // For text-based: open as text blob
      const blob = new Blob([active.html], { type: type === "json" ? "application/json" : type === "csv" ? "text/csv" : "text/plain" });
  ...
  const downloadFile = () => {
    ...
    const mimeMap: Record<string, string> = { html: "text/html", csv: "text/csv", json: "application/json", txt: "text/plain", markdown: "text/markdown", pdf: "application/pdf" };
    const extMap: Record<string, string> = { html: "html", csv: "csv", json: "json", txt: "txt", markdown: "md", pdf: "pdf" };
      ...
      <div className="flex items-center gap-2 px-4 py-2 shrink-0" style={{ borderBottom: "1px solid var(--c-border-1)" }}>
        ...
        {active && (
          <span
            ...
            style={{ background: TYPE_COLORS[activeType]?.bg, color: TYPE_COLORS[activeType]?.color }}
          >
                        ...
                        <span className="flex-1 min-w-0">
                          <span className="flex items-center gap-1">
                            <span
                              ...
                              style={{ background: TYPE_COLORS[entryType]?.bg, color: TYPE_COLORS[entryType]?.color }}
                            >
                          ...
                          <span className="block text-[9px] mt-0.5" style={{ color: "var(--c-text-5)" }}>
          ...
          {active ? (
            ...
            <div className="flex items-center justify-center h-full" style={{ color: "var(--c-text-4)" }}>
              <div className="text-center">
                ...
                <div className="text-xs" style={{ color: "var(--c-text-5)", maxWidth: 280, lineHeight: 1.6 }}>
                  {library.length > 0
                    ? "Select a preview from the library."
                    : (
                      <>
                        Supports <strong>HTML</strong>, <strong>CSV</strong>, <strong>JSON</strong>, <strong>TXT</strong>, and <strong>Markdown</strong>.<br />
                        Ask an agent to generate content, then click the Preview button on the code block.
                      </>
                    )}
                </div>
              </div>
