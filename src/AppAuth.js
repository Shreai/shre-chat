 ...
 */
export function useAuth(devBypass) {
    const devUser = {
        token: 'dev-token',
        user: { username: 'dev', name: 'Developer', role: 'admin' },
    };
    const [authState, setAuthState] = useState(() => devBypass ? devUser : getStoredAuth());
    const [authChecking, setAuthChecking] = useState(!devBypass);
    const [pendingWorkspaceSelection, setPendingWorkspaceSelection] = useState(null);
    useEffect(() => {
        if (devBypass)
            return;
    ...
    const handleLogin = useCallback((token, user, loginData) => {
    ...
    const handleWorkspaceSelected = useCallback(async (workspaceId) => {
 ...
 */
export function WorkspaceSelectionScreen({ pending, onSelect, }) {
    return (_jsx("div", { style: {
            ...
            background: 'var(--c-bg-1, #000)',
                ...
                maxWidth: 400,
                width: '90%',
            }, children: [_jsx("h2", { style: { color: 'var(--c-text-1, #fff)', fontSize: 18, fontWeight: 600, marginBottom: 8 }, children: "Select Workspace" }), _jsxs("p", { style: { color: 'var(--c-text-3, #888)', fontSize: 13, marginBottom: 20 }, children: ["Welcome, ", pending.user?.name, ". Choose a workspace to continue."] }), _jsx("div", { style: { display: 'flex', flexDirection: 'column', gap: 8 }, children: pending.workspaces.map((ws) => (_jsxs("button", { onClick: () => onSelect(ws.id), style: {
                            display: 'flex',
                            alignItems: 'center',
                        ...
                        }, onMouseOver: (e) => (e.currentTarget.style.background = 'var(--c-bg-4, #222)'), onMouseOut: (e) => (e.currentTarget.style.background = 'var(--c-bg-3, #1a1a1a)'), children: [_jsx("span", { children: ws.name }), _jsx("span", { style: { fontSize: 11, color: 'var(--c-text-3, #888)', textTransform: 'uppercase' }, children: ws.role })] }, ws.id))) })] }) }));
}
