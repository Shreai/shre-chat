...
import { loadSessions, syncWithServer, saveActiveSession, flushPendingSave, saveSessionImmediate, fetchAgentModels, fetchAgentRegistry, fetchAgentCapabilities, initStorage, debouncedSaveSessions, markSessionDirty, } from './store';
...
export function useThemeCustomEffect(themeCustom) {
    useEffect(() => {
...
export function useInitEffects(sessions, setSessions, setActiveSessionId, setSyncing) {
    useEffect(() => {
        fetchAgentModels();
...
export function useStreamPersistence(activeSessionId, streamingRef, streamTextRef) {
    useEffect(() => {
...
export function usePeriodicSync(activeSessionId, sessionsRef) {
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            if (s)
                saveSessionImmediate(s);
        }, 30_000);
        return () => clearInterval(interval);
    }, [activeSessionId, sessionsRef]);
}
// ── Daily session compaction ──
export function useDailyCompaction(sessions) {
    useEffect(() => {
        const COMPACT_DATE_KEY = 'shre-last-compact';
        const today = new Date().toISOString().slice(0, 10);
        if (localStorage.getItem(COMPACT_DATE_KEY) === today)
            return;
...
export function useCrossTabSync(crossTabRef, setSessions, setActivity, setFeed, setFiles, setOpenTabs) {
    useEffect(() => {
        const handler = (e) => {
            if (!e.key)
                return;
...
export function useViewSwitchEvent(setView) {
    useEffect(() => {
        const handler = (e) => {
            const v = e.detail;
            if (v)
...
export function useFoldDetection(actions) {
    useEffect(() => {
...
export function useUpdateSessions(setSessions, crossTabRef) {
    return useCallback((fn) => {
        setSessions((prev) => {
            ...
            if (!crossTabRef.current) {
                ...
                for (const s of next) {
                    if (s.updatedAt !== prevMap.get(s.id))
...
export function useSwipeGesture(sidebarOpen, actions) {
