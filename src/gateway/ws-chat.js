 ...
 */
export async function setModelWS(modelId, agentId = 'main') {
    try {
...
export async function sendChatWS(agentId, sessionKey, message, callbacks, modelOverride, systemPrompt) {
 ...
 */
export async function abortChatWS(agentId, sessionKey) {
    const streamKey = `${agentId}:${sessionKey}`;
    activeStreams.delete(streamKey);
    notifyStreamChange();
    try {
 ...
 */
export async function loadHistoryWS(agentId, sessionKey, limit = 200) {
