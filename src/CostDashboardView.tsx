...
interface CostByModel { model: string; provider: string; requests: number; costUsd: number; pct: number; }
interface CostByAgent { agent: string; requests: number; costUsd: number; savingsUsd: number; }
interface TimelinePoint { bucket: string; costUsd: number; requests: number; }
interface BudgetInfo { agentId: string; dailyLimitUsd: number; weeklyLimitUsd: number; spentTodayUsd: number; spentWeekUsd: number; }
...
function StatCard({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="rounded-lg px-3 py-2.5" style={{ background: "var(--c-bg-2)", border: "1px solid var(--c-border-2)" }}>
      <div className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: "var(--c-text-5)" }}>{label}</div>
      <div className="text-lg font-bold" style={{ color: accent || "var(--c-text-1)" }}>{value}</div>
    </div>
  );
}
...
export function CostDashboardView() {
  ...
  const timelineChart: ChartData | null = timeline.length ? {
    type: "line", labels: timeline.map(t => t.bucket.slice(5, 10)),
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden" style={{ background: "var(--c-bg-1)" }}>
      <div className="px-4 py-3 flex items-center gap-2 justify-between" style={{ borderBottom: "1px solid var(--c-border-1)" }}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold" style={{ color: "var(--c-text-1)" }}>Cost Dashboard</span>
          <span className="text-[10px] font-mono" style={{ color: "var(--c-text-5)" }}>via shre-meter</span>
        </div>
        <div className="flex items-center gap-2">
        ...
        {!loading && summary && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              ...
              <StatCard label="Savings (Local)" value={fmtUsd(summary.totalSavingsUsd)} accent="#4ade80" />
            ...
            {timelineChart && (
              <div>
                <h3 className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--c-text-5)" }}>Spend Timeline</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {modelChart && (
                <div>
                  <h3 className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--c-text-5)" }}>By Model</h3>
                  <div className="rounded-lg p-3" style={{ background: "var(--c-bg-2)", border: "1px solid var(--c-border-2)" }}>
              ...
              {agentChart && (
                <div>
                  <h3 className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--c-text-5)" }}>By Agent</h3>
                  <div className="rounded-lg p-3" style={{ background: "var(--c-bg-2)", border: "1px solid var(--c-border-2)" }}>
            ...
            {budgets.length > 0 && (
              <div>
                <h3 className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--c-text-5)" }}>Budget Status</h3>
                <div className="space-y-1.5">
                  {budgets.map((b) => {
                    const dailyPct = b.dailyLimitUsd > 0 ? Math.min(100, (b.spentTodayUsd / b.dailyLimitUsd) * 100) : 0;
                    const weeklyPct = b.weeklyLimitUsd > 0 ? Math.min(100, (b.spentWeekUsd / b.weeklyLimitUsd) * 100) : 0;
                    return (
                      <div key={b.agentId} className="rounded-lg px-3 py-2" style={{ background: "var(--c-bg-2)", border: "1px solid var(--c-border-2)" }}>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-[11px] font-medium" style={{ color: "var(--c-text-2)" }}>{b.agentId}</span>
                          <span className="text-[10px]" style={{ color: "var(--c-text-4)" }}>
                            {fmtUsd(b.spentTodayUsd)} / {fmtUsd(b.dailyLimitUsd)} daily
                          </span>
                        </div>
                        <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "var(--c-bg-1)" }}>
