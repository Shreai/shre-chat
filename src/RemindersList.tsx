    ...
    return (
      <div className="px-3 py-3 rounded-xl animate-pulse" style={{ background: 'var(--c-bg-2)' }}>
        ...
        <div
          ...
          style={{ background: 'var(--c-bg-hover)', width: '70%' }}
        />
      </div>
    );
  }
    ...
    <div className="space-y-1">
      ...
      {sorted.slice(0, 5).map((r) => {
        ...
        return (
          ...
          >
            <span
              className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${isOverdue ? 'bg-red-500' : 'bg-green-500'}`}
            />
            <span className="flex-1 text-xs truncate" style={{ color: 'var(--c-text-1)' }}>
              {r.text}
            </span>
            <span
