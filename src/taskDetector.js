...
export function detectTaskIntent(message) {
    ...
    for (const pattern of TASK_PATTERNS) {
        ...
        if (match) {
            ...
            if (!rawTitle || rawTitle.split(/\s+/).length < 1)
                continue;
 ...
 */
export function detectIssueIntent(message) {
    const trimmed = message.trim();
    if (!trimmed || trimmed.length < 5)
        return null;
 ...
 */
export async function createIssueFromChat(title, description, priority) {
    const now = Date.now();
 ...
 */
export async function createTaskFromChat(title, description) {
    // Cooldown check
    const now = Date.now();
