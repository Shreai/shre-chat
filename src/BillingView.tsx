...
export function BillingView() {
      ...
      try {
        ...
        if (billingRes.status === "fulfilled" && billingRes.value) {
          setBilling({
            ...
            status: billingRes.value.expired ? "expired" : billingRes.value.active ? "trialing" : "active",
  ...
  return (
    <div className="flex-1 overflow-y-auto" style={{ background: "var(--c-bg-1)" }}>
      {/* Header */}
      <div className="px-6 pt-6 pb-4 flex items-center justify-between" style={{ borderBottom: "1px solid var(--c-border-2)" }}>
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold" style={{ color: "var(--c-text-1)" }}>Billing</h1>
        </div>
        <button
      ...
      <div className="p-6 space-y-6 max-w-4xl mx-auto">
        {error && (
          <div className="px-4 py-2.5 rounded-lg text-sm" style={{ background: "rgba(239,68,68,0.1)", color: "#f87171", border: "1px solid rgba(239,68,68,0.25)" }}>
            {error}
          </div>
        )}

        {/* Current Plan Card */}
        <div className="rounded-xl p-5" style={{ background: "var(--c-bg-2)", border: "1px solid var(--c-border-1)" }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-sm font-semibold" style={{ color: "var(--c-text-1)" }}>{planMeta.name} Plan</h2>
        ...
        {usage && (
          <div className="grid grid-cols-3 gap-3">
            <StatCard label="Total Spend" value={fmtUsd(usage.totalCostUsd)} sub={`${fmtDate(usage.periodFrom)} - ${fmtDate(usage.periodTo)}`} />
...
function StatCard({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div className="rounded-xl p-4" style={{ background: "var(--c-bg-2)", border: "1px solid var(--c-border-1)" }}>
      ...
      <p className="text-lg font-semibold mt-1" style={{ color: "var(--c-text-1)" }}>{value}</p>
      <p className="text-[10px] mt-1" style={{ color: "var(--c-text-5)" }}>{sub}</p>
    </div>
  );
}
