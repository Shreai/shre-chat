    ...
    >
      ...
      {showPlans && (
        <div
          ...
          style={{ background: "var(--c-bg-2)", border: "1px solid var(--c-border-1)", minWidth: 260 }}
        >
          {PLANS.map((plan) => (
            <button
              key={plan.id}
            ...
            >
              <div>
                ...
                <div className="text-xs" style={{ color: "var(--c-text-4)" }}>{plan.desc}</div>
              </div>
              <span className="text-xs font-medium" style={{ color: "var(--c-accent)" }}>{plan.price}</span>
            </button>
          ))}
          {error && (
            <div className="px-4 py-2 text-xs" style={{ color: "#f87171" }}>{error}</div>
          )}
        </div>
      )}
    </div>
