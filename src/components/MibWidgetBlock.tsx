...
function BarChart({ block }: { block: Block }) {
  ...
  return (
    <svg width="100%" height={h + 20} viewBox={`0 0 ${labels.length * (barW + 8) + 16} ${h + 20}`} style={{ display: "block" }}>
      {values.map((v, i) => {
        const barH = (v / max) * h;
        const x = 8 + i * (barW + 8);
        return (
          <g key={i}>
            ...
            <text x={x + barW / 2} y={h + 14} fill="var(--c-text-4)" fontSize={9} textAnchor="middle">
              {(labels[i] ?? "").slice(0, 6)}
            </text>
          </g>
        );
...
function PieChart({ block }: { block: Block }) {
  ...
  if (entries.length === 0) return <div style={{ fontSize: 12, color: "var(--c-text-4)", padding: 8 }}>No data</div>;
      ...
      {entries.map(({ v, i: origIdx }, sliceIdx) => {
        ...
        return (
          <path key={sliceIdx} d={pathD} fill={PALETTE[sliceIdx % PALETTE.length]} opacity={0.85}>
            <title>{labels[origIdx]}: {v} ({((v / total) * 100).toFixed(1)}%)</title>
          </path>
        );
      })}
    </svg>
    ...
    <div style={{ overflowX: "auto", fontSize: 12 }}>
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr>
            {headers.map(h => (
              <th key={h} style={{ textAlign: "left", padding: "5px 8px", fontWeight: 600, borderBottom: "1px solid var(--c-border-2)", whiteSpace: "nowrap", color: "var(--c-text-4)" }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {visible.map((row, i) => (
            <tr key={i}>
              {headers.map((_, ci) => (
                <td key={ci} style={{ padding: "4px 8px", borderBottom: "1px solid var(--c-border-1)", whiteSpace: "nowrap", color: "var(--c-text-2)" }}>{row[ci] ?? ""}</td>
              ))}
            </tr>
          ))}
        </tbody>
      </table>
      {rows.length > 20 && <div style={{ padding: "4px 8px", fontSize: 11, color: "var(--c-text-4)", fontStyle: "italic" }}>+{rows.length - 20} more rows</div>}
    </div>
  );
}
...
function TodoWidget({ block }: { block: Block }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 10, color: "var(--c-text-4)", marginBottom: 4 }}>{done}/{items.length} complete</div>
      {items.map((item, i) => (
        <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "2px 0", cursor: editable ? "pointer" : "default" }}
          onClick={() => editable && setItems(prev => prev.map((it, j) => j === i ? { ...it, done: !it.done } : it))}>
          <span style={{ fontSize: 14 }}>{item.done ? "\u2705" : "\u2B1C"}</span>
          <span style={{ textDecoration: item.done ? "line-through" : "none", opacity: item.done ? 0.5 : 1, fontSize: 13, color: "var(--c-text-2)" }}>{item.text}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", alignItems: "baseline", gap: 8, padding: "4px 0" }}>
      <span style={{ fontSize: 24, fontWeight: 700, color: "var(--c-text-1)" }}>
        {value}{unit && <span style={{ fontSize: 14, color: "var(--c-text-4)", marginLeft: 2 }}>{unit}</span>}
      </span>
      {change !== undefined && (
...
function LinkCardWidget({ block }: { block: Block }) {
  ...
  return (
    <a href={isSafeUrl(url) ? url : "#"} target="_blank" rel="noopener noreferrer" style={{ display: "flex", gap: 10, padding: 8, borderRadius: 8, border: "1px solid var(--c-border-2)", textDecoration: "none", color: "inherit" }}>
      {image && isSafeUrl(image) && <img src={image} alt="" style={{ width: 60, height: 60, objectFit: "cover", borderRadius: 6 }} />}
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 2, color: "var(--c-text-1)" }}>{title}</div>
        {description && <div style={{ fontSize: 11, color: "var(--c-text-4)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{description}</div>}
...
function ImageGalleryWidget({ block }: { block: Block }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(images.length, 3)}, 1fr)`, gap: 4 }}>
      {images.slice(0, 9).map((img, i) => (
        <img key={i} src={img.src} alt={img.alt ?? ""} style={{ width: "100%", height: 80, objectFit: "cover", borderRadius: 4 }} />
      ))}
    </div>
  );
}
    ...
    <div style={{ overflowX: "auto", fontSize: 12 }}>
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr>
            {columns.map(col => (
              <th key={col.key} style={{ textAlign: "left", padding: "5px 8px", fontWeight: 600, borderBottom: "1px solid var(--c-border-2)", whiteSpace: "nowrap", color: "var(--c-text-4)" }}>{col.label}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {visible.map((row, i) => (
            <tr key={i}>
              {columns.map(col => (
                <td key={col.key} style={{ padding: "4px 8px", borderBottom: "1px solid var(--c-border-1)", whiteSpace: "nowrap", color: "var(--c-text-2)" }}>{String(row[col.key] ?? "")}</td>
              ))}
            </tr>
          ))}
        </tbody>
...
const WEATHER_ICONS: Record<string, string> = {
  ...
  cloudy: "\u2601\uFE0F", rain: "\uD83C\uDF27\uFE0F", thunderstorm: "\u26C8\uFE0F",
  snow: "\uD83C\uDF28\uFE0F", fog: "\uD83C\uDF2B\uFE0F", windy: "\uD83D\uDCA8",
};
...
function WeatherWidget({ block }: { block: Block }) {
  ...
  const forecast = (block.forecast as Array<{ day: string; high: number; low: number; condition: string }>) ?? [];

  return (
    <div>
      <div style={{ fontSize: 13, fontWeight: 600, color: "var(--c-text-1)", marginBottom: 4 }}>{location}</div>
      {current && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
          <span style={{ fontSize: 28, fontWeight: 700, color: "var(--c-text-1)" }}>{current.temp}°</span>
          <span style={{ fontSize: 20 }}>{WEATHER_ICONS[current.condition.toLowerCase()] ?? "\u2600\uFE0F"}</span>
          <span style={{ fontSize: 12, color: "var(--c-text-4)", textTransform: "capitalize" }}>{current.condition}</span>
        </div>
      )}
      {forecast.length > 0 && (
        <div style={{ display: "flex", gap: 6 }}>
          {forecast.slice(0, 7).map((day, i) => (
            <div key={i} style={{ textAlign: "center", padding: "4px 6px", borderRadius: 6, background: "var(--c-bg-3)", fontSize: 10 }}>
              <div style={{ fontWeight: 600, color: "var(--c-text-3)" }}>{day.day}</div>
              <div style={{ fontSize: 16 }}>{WEATHER_ICONS[day.condition.toLowerCase()] ?? "\u2600\uFE0F"}</div>
      ...
      <div style={{ padding: 10 }}>
        ...
        {block.type === "iframe" && isSafeUrl(block.src as string) && (
          <iframe
            ...
            style={{ width: "100%", height: (block.height as number) ?? 300, border: "none", borderRadius: 6 }}
