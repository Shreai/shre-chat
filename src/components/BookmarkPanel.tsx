...
import { getBookmarks, removeBookmark, updateBookmarkNote, getAgent, type Bookmark } from "../store";
...
export function BookmarkPanel({ open, onClose, onNavigate }: BookmarkPanelProps) {
  ...
  const refresh = useCallback(() => { setBookmarks(getBookmarks()); setLimit(PAGE_SIZE); }, []);
    ...
    <>
      ...
      >
        ...
        <div style={{
          ...
          <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--c-accent, #6366f1)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {bookmarks.length > 0 && (
            <button
              ...
              style={{
                background: "none", border: "none", color: "var(--c-text-4, #888)",
          ...
          <button
            ...
            style={{ background: "none", border: "none", color: "var(--c-text-4, #888)", cursor: "pointer", padding: 4, borderRadius: 6 }}
          >
        ...
        <div style={{ padding: "8px 12px" }}>
          <input
            ...
            style={{
              ...
              background: "var(--c-bg-card, var(--c-bg-3, #1a1a2e))", color: "var(--c-text-2, #ccc)",
              border: "1px solid var(--c-border-1, #333)", outline: "none",
        ...
        <div role="list" style={{ flex: 1, overflowY: "auto", padding: "4px 8px" }}>
          {filtered.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 12px", color: "var(--c-text-4, #888)" }}>
              <svg aria-hidden="true" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.4, margin: "0 auto 8px" }}>
                <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/>
              </svg>
            ...
            <>
              {visible.map(bm => {
                ...
                return (
                  <div
                    key={bm.id}
                    role="listitem"
                    onClick={() => { if (!isEditing) { onNavigate(bm.sessionId, bm.messageIndex); onClose(); } }}
                    style={{
                    ...
                    onMouseEnter={e => { if (!isDeleting) { e.currentTarget.style.background = "var(--c-bg-hover, rgba(255,255,255,0.08))"; e.currentTarget.style.borderColor = "var(--c-border-1, #333)"; } }}
                    onMouseLeave={e => { e.currentTarget.style.background = "var(--c-bg-3, rgba(255,255,255,0.04))"; e.currentTarget.style.borderColor = "var(--c-border-2, #2a2a3d)"; }}
                  >
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                      <span style={{ fontSize: 14, flexShrink: 0 }}>{agent.emoji}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12, color: "var(--c-text-1, #eee)", lineHeight: 1.4, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                          {bm.preview}
                        </div>
                        {/* Inline note editing */}
                        {isEditing ? (
                          <input
                            ...
                            style={{
                              ...
                              fontSize: 11, color: "var(--c-accent, #6366f1)",
                              background: "var(--c-bg-2, #0f0f1a)",
                          ...
                          <div
                            onClick={e => { e.stopPropagation(); setEditingId(bm.id); setEditNote(bm.note || ""); }}
                            title="Click to edit note"
                            style={{
                              fontSize: 11, marginTop: 3, cursor: "text", minHeight: 16,
                              color: bm.note ? "var(--c-accent, #6366f1)" : "var(--c-text-5, #666)",
                              borderLeft: bm.note ? "2px solid var(--c-accent, #6366f1)" : "2px solid transparent",
                              paddingLeft: 6, fontStyle: bm.note ? "normal" : "italic",
                            }}
                          >
                            {bm.note || "Add note..."}
                          </div>
                        )}
                        <div style={{ fontSize: 10, color: "var(--c-text-5, #666)", marginTop: 4, display: "flex", alignItems: "center", gap: 6 }}>
                          <span>{agent.name}</span>
                          <span>&middot;</span>
                          <span>{new Date(bm.createdAt).toLocaleDateString([], { month: "short", day: "numeric" })}</span>
                        </div>
                      </div>
                      <button
                        onClick={e => { e.stopPropagation(); handleDelete(bm.id); }}
                        style={{ background: "none", border: "none", color: "var(--c-text-5, #666)", cursor: "pointer", padding: 2, borderRadius: 4, flexShrink: 0 }}
                        aria-label={`Remove bookmark for: ${bm.preview.slice(0, 40)}`}
                      >
                        <svg aria-hidden="true" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M18 6L6 18M6 6l12 12"/></svg>
                      </button>
                    </div>
                  </div>
                );
              ...
              {hasMore && (
                <button
                  ...
                  style={{
                    width: "100%", padding: "8px", marginTop: 4, marginBottom: 8,
                    fontSize: 12, color: "var(--c-accent, #6366f1)", fontWeight: 500,
                    background: "none", border: "1px solid var(--c-border-2, #2a2a3d)",
