    ...
    <div className="max-w-5xl mx-auto mt-4 mb-4">
      <div className="flex items-center gap-2 mb-3 px-1">
        ...
        {Object.values(compareStreams).every((s) => s.done) && !compareWinner && (
          <span className="text-[10px] ml-2" style={{ color: 'var(--c-text-4)' }}>
            Pick a winner to save as the response
          </span>
        )}
        {compareWinner && (
          <span
            className="text-[10px] ml-2 px-2 py-0.5 rounded-full"
            style={{ background: 'rgba(34,197,94,0.15)', color: 'var(--c-success)' }}
          >
            Winner:{' '}
            {availableModels.find((m) => m.id === compareWinner)?.name ||
              compareWinner.split('/')[1]}
          </span>
        )}
      </div>
      <div
        className="grid gap-3"
        style={{ gridTemplateColumns: `repeat(${Object.keys(compareStreams).length}, 1fr)` }}
      >
        {Object.entries(compareStreams).map(([modelId, stream]) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className="flex items-center gap-1.5">
                  ...
                  <span
                    ...
                    style={{ color: isWinner ? 'var(--c-success)' : 'var(--c-text-2)' }}
                  >
                  ...
                  {stream.done ? (
                    ...
                    <span className="flex gap-0.5 items-center">
                      <span
                        ...
                        style={{ background: 'var(--c-warning)', animationDelay: '0ms' }}
                      />
                      <span
                        className="h-1.5 w-1.5 rounded-full animate-bounce"
                        style={{ background: 'var(--c-warning)', animationDelay: '150ms' }}
                      />
                      <span
                        className="h-1.5 w-1.5 rounded-full animate-bounce"
                        style={{ background: 'var(--c-warning)', animationDelay: '300ms' }}
                      />
                    </span>
                  )}
                </div>
              ...
              {/* Response content */}
              <div
                className="flex-1 px-3 py-2 overflow-y-auto text-sm leading-relaxed"
                style={{ color: 'var(--c-text-1)', maxHeight: '400px' }}
              >
