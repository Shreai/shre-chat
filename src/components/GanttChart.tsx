  ...
  const { minTime, maxTime, rows } = useMemo(() => {
    ...
    return {
      ...
      rows: tasks.map((t) => ({
        ...
        w: Math.max(
          0.01,
          (new Date(t.end || t.start).getTime() - new Date(t.start).getTime()) / span,
        ),
      })),
    };
  }, [tasks]);
      ...
      {ticks.map((t, i) => (
        <g key={i}>
          <line
            ...
            strokeWidth="1"
          />
          <text x={t.x} y={14} textAnchor="middle" fill="rgba(255,255,255,0.4)" fontSize="9">
            {t.label}
          </text>
        </g>
      ))}
          ...
          >
            ...
            {isHovered && (
              <g>
                ...
                />
                <text x={barX + barW + 10} y={y + 14} fill="#fff" fontSize="10" fontWeight="600">
                  {task.title}
                </text>
