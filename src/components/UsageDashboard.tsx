...
export function UsageDashboard({ standalone }: { standalone?: boolean } = {}) {
  ...
  return (
    <div
      className={
        standalone ? 'flex-1 overflow-y-auto p-6 max-w-4xl mx-auto space-y-6' : 'space-y-5'
      }
    >
      {/* Period selector */}
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium" style={{ color: 'var(--c-text-2)' }}>
          Usage Breakdown
        </h3>
      ...
      {summary && (summary.localPercent > 0 || summary.cloudPercent > 0) && (
        <div
          ...
          style={{ background: 'var(--c-bg-2)', border: '1px solid var(--c-border-1)' }}
        >
          <h4 className="text-xs font-medium mb-2" style={{ color: 'var(--c-text-3)' }}>
            Local vs Cloud
          </h4>
          <div
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return (
    ...
    >
      ...
      <p className="text-base font-semibold mt-0.5" style={{ color: 'var(--c-text-1)' }}>
        {value}
      </p>
    </div>
  );
