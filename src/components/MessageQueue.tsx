...
export function MessageQueue({ queue, editingQueueId, onReorder, onEdit, onRemove }: MessageQueueProps) {
    ...
    <div className="px-4 py-2 shrink-0" style={{ borderTop: "1px solid var(--c-border-2)" }}>
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center gap-2 mb-1.5">
          <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: "var(--c-text-5)" }}>Queue</span>
          <span className="text-[9px] px-1.5 rounded-full" style={{ background: "var(--c-bg-badge)", color: "var(--c-text-2)" }}>{queue.length}</span>
        </div>
        <div className="flex flex-col gap-1">
          {queue.map((item, idx) => (
            <div key={item.id} className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 group"
              style={{ background: "var(--c-bg-card)", border: "1px solid var(--c-border-2)" }}>
              {/* Priority number */}
              <span className="text-[10px] font-mono shrink-0 w-4 text-center" style={{ color: "var(--c-text-5)" }}>{idx + 1}</span>
              {/* Text */}
              <span className="flex-1 text-xs truncate" style={{ color: editingQueueId === item.id ? "var(--c-accent)" : "var(--c-text-2)" }}>{item.text}</span>
              ...
              <div className="hidden group-hover:flex items-center gap-0.5 shrink-0">
                ...
                <button
                  onClick={() => onEdit(item)}
                  className="p-0.5 rounded transition-colors"
                  style={{ color: editingQueueId === item.id ? "var(--c-accent)" : "var(--c-text-4)" }}
                  title="Edit"
                >
