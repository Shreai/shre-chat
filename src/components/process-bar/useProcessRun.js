...
function reducer(state, action) {
    switch (action.type) {
        ...
        case 'ADD_STEP':
            return {
                runs: state.runs.map((r) => r.id === action.runId ? { ...r, steps: [...r.steps, action.step] } : r),
            };
        case 'UPDATE_STEP':
            return {
                runs: state.runs.map((r) => r.id === action.runId
                    ? {
                        ...r,
                        steps: r.steps.map((s) => s.id === action.stepId ? { ...s, ...action.patch, updatedAt: Date.now() } : s),
                    }
                    : r),
            };
        case 'COMPLETE_RUN':
            return {
                runs: state.runs.map((r) => r.id === action.runId
                    ? {
                        ...
                        steps: r.steps.map((s) => s.status === 'active'
...
export function useProcessRun() {
    ...
    const startRun = useCallback((runId, sessionId) => {
        dispatch({ type: 'START_RUN', runId, sessionId });
    }, []);
    const addStep = useCallback((runId, opts) => {
    ...
    const updateStep = useCallback((runId, stepId, patch) => {
        dispatch({ type: 'UPDATE_STEP', runId, stepId, patch });
    }, []);
    const completeRun = useCallback((runId, meta) => {
