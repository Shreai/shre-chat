...
// ---------------------------------------------------------------------------
function StageNode({ step, isLast }) {
    ...
    return (_jsxs("div", { style: {
            ...
            flexShrink: 0,
                    ...
                    position: 'relative',
                            ...
                            textAlign: 'center',
                        }, children: step.toolName || config.label }), step.completedAt && (_jsxs("span", { style: {
                            fontSize: '8px',
                            color: 'var(--c-text-5)',
                            lineHeight: 1,
                        }, children: [((step.completedAt - step.startedAt) / 1000).toFixed(1), "s"] }))] }), !isLast && _jsx(Connector, { completed: isCompleted })] }));
}
export function ExecutionTimeline({ run, className, onExpand }) {
    ...
    return (_jsxs("div", { className: className, onClick: onExpand, style: {
            display: 'flex',
            flexDirection: 'column',
            gap: '6px',
            padding: '8px 12px',
            ...
            marginBottom: '4px',
                    ...
                    borderBottom: '1px solid var(--c-border-2, rgba(255,255,255,0.06))',
                        ...
                        }, children: isComplete ? 'Execution Complete' : 'Executing...' }), _jsxs("div", { style: {
                            display: 'flex',
                            alignItems: 'center',
                            gap: '6px',
                        }, children: [run.model && (_jsx("span", { style: { fontSize: '9px', color: 'var(--c-text-5)', fontFamily: 'monospace' }, children: run.model.split('/').pop() })), _jsxs("span", { style: {
                                    fontSize: '9px',
                                    color: 'var(--c-text-5)',
                                    fontFamily: 'monospace',
                                }, children: [(elapsed / 1000).toFixed(1), "s"] }), onExpand && _jsx("span", { style: { fontSize: '10px', color: 'var(--c-text-5)' }, children: "\u25B8" })] })] }), _jsx("div", { style: {
                    display: 'flex',
                    ...
                    overflowX: 'auto',
                    paddingBottom: '2px',
                }, children: run.steps.map((step, i) => (_jsx(StageNode, { step: step, isLast: i === run.steps.length - 1 }, step.id))) })] }));
}
