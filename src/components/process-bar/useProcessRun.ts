...
type Action =
  ...
  | { type: "COMPLETE_RUN"; runId: string; meta?: { model?: string; tokenUsage?: { input: number; output: number }; durationMs?: number } }
  | { type: "CLEAR" };
...
export function useProcessRun() {
  ...
  const addStep = useCallback(
    (runId: string, opts: { kind: ProcessStepKind; label: string; toolName?: string; toolArgs?: unknown; detail?: string }) => {
      const id = `step-${++stepCounter}`;
      const now = Date.now();
      const step: ProcessStep = {
        id, kind: opts.kind, label: opts.label, toolName: opts.toolName,
        toolArgs: opts.toolArgs, detail: opts.detail,
  ...
  const updateStep = useCallback(
    (runId: string, stepId: string, patch: Partial<ProcessStep>) => {
  ...
  const completeRun = useCallback(
    (runId: string, meta?: { model?: string; tokenUsage?: { input: number; output: number }; durationMs?: number }) => {
  ...
  const activeRun = useMemo(
    () => state.runs.find((r) => !r.completedAt) ?? null,
    [state.runs],
  );
