 ...
 */
function generateLocalSuggestions(message) {
    ...
    if (results.length === 0)
        return DEFAULT_SUGGESTIONS.slice(0, MAX_VISIBLE);
    return results;
}
// ── Component ────────────────────────────────────────────────────────
export function SuggestionsBar({ lastAssistantMessage, streaming, onSelect, suggestions: externalSuggestions, messageCount = 0, }) {
    ...
    useEffect(() => {
        ...
        if (lastAssistantMessage === prevMessageRef.current)
            return;
    ...
    const displaySuggestions = isEmptyChat
        ...
        : externalSuggestions && externalSuggestions.length > 0
            ? externalSuggestions
            : localSuggestions;
    if (displaySuggestions.length === 0 || streaming || !visible)
        return null;
    return (_jsx("div", { className: "suggestions-bar scrollbar-none", style: {
            display: 'flex',
            overflowX: 'auto',
            gap: 8,
            padding: '6px 16px',
            ...
            WebkitMaskImage: 'linear-gradient(to right, transparent 0, black 16px, black calc(100% - 24px), transparent 100%)',
        }, children: displaySuggestions.slice(0, MAX_VISIBLE).map((suggestion, i) => (_jsx("button", { className: "suggestion-chip", style: {
                ...
                if (fadeTimerRef.current)
                    clearTimeout(fadeTimerRef.current);
                onSelect(suggestion);
            }, children: suggestion }, `${suggestion}-${i}`))) }));
}
