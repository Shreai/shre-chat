    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} ${compact ? "max-w-2xl" : "max-w-3xl"} mx-auto`}>
      ...
      >
                ...
                components={{
                  ...
                  code({ className, children, ...props }) {
                    ...
                    if (lang === "mib-widget") {
                      ...
                      } catch (err) { console.debug("widget JSON parse", err); }
                    }
          ...
          <div className="flex items-center gap-1 mt-0.5 px-1">
            ...
            {routeExpanded && (
              <div
                ...
                style={{ background: "var(--c-bg-3)", color: "var(--c-text-4)", border: "1px solid var(--c-border-2)" }}
              >
        ...
        {!streaming && message.content && !editing && (
          isUser ? (
            <div className="flex items-center justify-end gap-0.5 mt-1 px-1">
              ...
              {onAnnotate && (
                <button onClick={() => { setAnnotationEditing(true); setAnnotationDraft(message.annotation || ""); }} className="p-1 rounded transition-colors opacity-0 group-hover/msg:opacity-100" style={{ color: message.annotation ? "var(--c-accent)" : "var(--c-text-2)" }} title={message.annotation ? "Edit annotation" : "Add annotation"}>
              ...
              {onEditStart && (
                <button onClick={onEditStart} className="p-1 rounded transition-colors opacity-0 group-hover/msg:opacity-100 hover:bg-white/5" style={{ color: "var(--c-text-2)" }} title="Edit message">
              ...
              {onBranch && (
                <button onClick={onBranch} className="p-1 rounded transition-colors opacity-0 group-hover/msg:opacity-100 hover:bg-white/5" style={{ color: "var(--c-text-2)" }} title="Branch conversation here">
              ...
              {onReaction && (
                <div ref={reactionPickerRef} style={{ position: "relative", display: "inline-block" }}>
                  <button onClick={() => setReactionPickerOpen((o) => !o)} className="p-1 rounded transition-colors opacity-0 group-hover/msg:opacity-100 hover:bg-white/5" style={{ color: reactionPickerOpen ? "var(--c-accent)" : "var(--c-text-2)" }} title="Add reaction">
                  ...
                  {reactionPickerOpen && (
                    <div style={{ position: "absolute", bottom: "calc(100% + 4px)", right: 0, background: "var(--c-bg-2)", border: "1px solid var(--c-border-1)", borderRadius: "12px", padding: "4px 6px", display: "flex", gap: "2px", zIndex: 50, boxShadow: "0 4px 12px rgba(0,0,0,0.15)", whiteSpace: "nowrap" }}>
                      {REACTION_EMOJIS.map((emoji) => (
                        <button key={emoji} onClick={() => { onReaction(emoji); setReactionPickerOpen(false); }} className="rounded transition-transform hover:scale-125" style={{ padding: "2px 4px", fontSize: "16px", lineHeight: 1, background: "transparent", border: "none", cursor: "pointer" }} title={emoji}>{emoji}</button>
                      ))}
                    </div>
                  )}
                </div>
              ...
              {onReply && (
                <button onClick={onReply} className="p-1 rounded transition-colors opacity-0 group-hover/msg:opacity-100" style={{ color: 'var(--c-text-5)' }} title="Reply to this message">
              ...
              {onToggleBookmark && (
                <button onClick={onToggleBookmark} className={`p-1 rounded transition-colors ${isBookmarked ? "opacity-100" : "opacity-0 group-hover/msg:opacity-100"}`} style={{ color: isBookmarked ? "var(--c-accent)" : "var(--c-text-5)" }} title={isBookmarked ? "Remove bookmark" : "Bookmark this message"}>
            ...
            <div>
              <div className="flex items-center gap-0.5">
                ...
                {onAnnotate && (
                  <button onClick={() => { setAnnotationEditing(true); setAnnotationDraft(message.annotation || ""); }} className="p-1 rounded transition-colors opacity-0 group-hover/msg:opacity-100" style={{ color: message.annotation ? "var(--c-accent)" : "var(--c-text-5)" }} title={message.annotation ? "Edit annotation" : "Add annotation"}>
                    <svg className="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
                  </button>
                )}
                {onToggleBookmark && (
        ...
        {!isUser && message.content.startsWith("Error:") && onRetry && (
          <button onClick={onRetry} className="mt-1.5 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all hover:brightness-125" style={{ background: "rgba(239,68,68,0.1)", color: "var(--c-danger-soft)", border: "1px solid rgba(239,68,68,0.2)" }}>
        ...
        {annotationEditing && onAnnotate && (
          <div className="mt-1 px-1">
            <div className="flex items-center gap-1">
              <input type="text" className="flex-1 rounded-lg px-2 py-1 text-xs focus:outline-none focus:ring-1" style={{ background: "var(--c-bg-2)", color: "var(--c-text-1)", border: "1px solid var(--c-border-1)" }} value={annotationDraft} onChange={(e) => setAnnotationDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { onAnnotate(annotationDraft); setAnnotationEditing(false); } if (e.key === "Escape") setAnnotationEditing(false); }} placeholder="Add a note..." autoFocus />
              <button onClick={() => { onAnnotate(annotationDraft); setAnnotationEditing(false); }} className="text-[10px] px-2 py-1 rounded-lg font-medium" style={{ color: "var(--c-on-accent)", background: "var(--c-accent)" }}>Save</button>
              <button onClick={() => setAnnotationEditing(false)} className="text-[10px] px-1.5 py-1 rounded-lg" style={{ color: "var(--c-text-4)" }}>Cancel</button>
            </div>
          </div>
        )}
        {/* Annotation display */}
        {!annotationEditing && message.annotation && (
          <div className="mt-1 mx-1 px-2 py-1 rounded-lg text-xs flex items-center gap-1 cursor-pointer" style={{ background: "color-mix(in srgb, var(--c-accent) 10%, transparent)", borderLeft: "2px solid var(--c-accent)", color: "var(--c-text-3)" }} onClick={() => { if (onAnnotate) { setAnnotationEditing(true); setAnnotationDraft(message.annotation || ""); } }} title="Click to edit annotation">
            ...
            {onAnnotate && (
              <button onClick={(e) => { e.stopPropagation(); onAnnotate(""); }} className="flex-shrink-0 p-0.5 rounded hover:bg-black/10 transition-colors" style={{ color: "var(--c-text-5)" }} title="Remove annotation">
        ...
        {message.reactions && Object.keys(message.reactions).length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1 px-1">
            {Object.entries(message.reactions).map(([emoji, count]) => (
              <button key={emoji} onClick={() => onReaction?.(emoji)} className="inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-xs transition-colors" style={{ background: "color-mix(in srgb, var(--c-accent) 12%, transparent)", border: "1px solid color-mix(in srgb, var(--c-accent) 25%, transparent)", color: "var(--c-text-2)", cursor: "pointer", lineHeight: 1.2 }} title={`${emoji} ${count}`}>
