...
function niceScale(min: number, max: number, ticks: number) {
  ...
  const nice =
    rough / mag >= 5 ? 10 * mag : rough / mag >= 2 ? 5 * mag : rough / mag >= 1 ? 2 * mag : mag;
...
function BarChart({
  ...
  return (
    <g>
      {scale.steps.map((v, i) => (
        <g key={i}>
          <line x1={pad.l} x2={w - pad.r} y1={toY(v)} y2={toY(v)} stroke={gridColor} />
          <text x={pad.l - 4} y={toY(v) + 3} fill={textColor} fontSize={10} textAnchor="end">
            {formatValue(v, data.options?.currency)}
          </text>
        </g>
      ))}
        ...
        return (
          <g key={li}>
              ...
              return (
                <g key={di}>
                  ...
                  >
                    <title>
                      {ds.label ? `${ds.label}: ` : ''}
                      {formatValue(val, data.options?.currency)}
                    </title>
                  </rect>
            ...
            <text
              ...
              transform={rot ? `rotate(-45, ${gx}, ${h - pad.b + 14})` : undefined}
            >
              {label}
            </text>
          </g>
      ...
      {data.labels.map((label, li) => {
        ...
        return (
          <text
            ...
            fontSize={10}
            textAnchor={rot ? 'end' : 'middle'}
            transform={rot ? `rotate(-45, ${x}, ${h - pad.b + 14})` : undefined}
          >
            {label}
          </text>
        );
          ...
          <g key={di}>
            {filled && (
              <>
                ...
                <path
                  d={`${path} L ${points[points.length - 1].x} ${toY(scale.min)} L ${points[0].x} ${toY(scale.min)} Z`}
                  fill={`url(#${gradId})`}
                />
              </>
            )}
            ...
            {points.map((p, pi) => (
              <circle key={pi} cx={p.x} cy={p.y} r={3} fill={color}>
                <title>
                  {ds.label ? `${ds.label}: ` : ''}
                  {formatValue(ds.data[pi], data.options?.currency)}
                </title>
              </circle>
            ))}
            {data.options?.showValues &&
...
function PieChart({
  ...
  if (entries.length === 0) {
    return (
      <text x={cx} y={cy} fill={textColor} fontSize={11} textAnchor="middle">
        No data
      </text>
    );
  }
    ...
    <g>
      {entries.map(({ v, i: origIdx }, sliceIdx) => {
        ...
        return (
          <g key={origIdx}>
            <path
              d={describeArc(cx, cy, r, start, start + sweep - 0.3)}
              fill={color || dsColor(data.datasets[0], sliceIdx)}
              opacity={0.85}
            >
              <title>
                {data.labels[origIdx]}: {formatValue(v, data.options?.currency)} ({pct})
              </title>
            </path>
            {sweep > 18 && (
              <text
            ...
            ? data.datasets.map((ds, i) => {
                ...
                return (
                  <g key={i} transform={`translate(${offset}, 0)`}>
                    ...
                    <text x={0} y={0} fill={textColor} fontSize={10}>
                      {ds.label || `Series ${i + 1}`}
                    </text>
                  </g>
                );
