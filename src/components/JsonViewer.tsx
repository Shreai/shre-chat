...
function JsonNode({
  ...
  if (data === null)
    return (
      <div
        style={{ paddingLeft: depth * 16, display: 'flex', alignItems: 'center' }}
        className="json-row"
      >
        {renderKey()}
  ...
  if (!expanded)
    return (
      ...
      >
        ...
        {renderKey()}
        <span style={{ color: colors.brace, cursor: 'pointer' }} onClick={() => setExpanded(true)}>
          {summary}
        </span>
        <CopyBtn value={data} />
      </div>
  ...
  return (
    <div>
      ...
      >
        <span
          style={{ cursor: 'pointer', marginRight: 4, userSelect: 'none' }}
          onClick={() => depth > 0 && setExpanded(false)}
        >
          &#9662;
        </span>
        {renderKey()}
