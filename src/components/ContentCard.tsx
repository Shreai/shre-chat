  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      if (e.origin === window.location.origin && e.data?.type === "iframe-height" && typeof e.data.height === "number") {
...
function TablePreview({ content }: { content: string }) {
  ...
  if (parseError || rows.length === 0 || !rows[0]) {
    return <pre style={{ color: "var(--c-text-3)", fontSize: 12, padding: 8 }}>Unable to parse table data</pre>;
  }
  ...
  return (
    <div style={{ overflowX: "auto", fontSize: 12 }}>
            ...
            {headers.map((h) => (
              <th
                ...
                style={{
                  ...
                  background: "rgba(0,0,0,0.4)", color: "var(--c-text-2)",
                  borderBottom: "1px solid var(--c-border-2)", textAlign: "left",
              ...
              {headers.map((h) => (
                <td
                  ...
                  style={{
                    ...
                    color: "var(--c-text-2)", whiteSpace: "nowrap",
                  }}
                >
                  {String(row[h] ?? "")}
                </td>
      ...
      {truncated && (
        <div style={{ padding: "8px 10px", color: "var(--c-text-3)", fontSize: 11, fontStyle: "italic" }}>
...
function JsonPreview({ content }: { content: string }) {
  ...
  if (error) {
    return (
      <div style={{ padding: 8 }}>
        ...
        <pre style={{ color: "var(--c-text-3)", fontSize: 12, whiteSpace: "pre-wrap", wordBreak: "break-all" }}>
          {content}
        </pre>
      </div>
    );
  ...
  return (
    <Suspense fallback={<div style={{ padding: 12, color: "var(--c-text-3)", fontSize: 12 }}>Loading...</div>}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: 12, fontWeight: 500, color: "var(--c-text-2)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {displayTitle}
        </span>
        {onExpand && (
          <button
      ...
      >
        <button
          onClick={handleCopy}
          style={{
            background: copied ? "rgba(52,211,153,0.15)" : "var(--c-bg-hover, rgba(255,255,255,0.06))",
