    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold" style={{ color: "var(--c-text-1)" }}>Keyboard Shortcuts</h2>
        ...
        <div className="space-y-1 text-xs">
          {SHORTCUTS.map(([key, desc]) => (
            <div key={key} className="flex items-center justify-between py-1.5 px-2 rounded-lg" style={{ background: "var(--c-bg-3)" }}>
              <span style={{ color: "var(--c-text-3)" }}>{desc}</span>
              <kbd className="px-2 py-0.5 rounded text-[10px] font-mono font-medium" style={{ background: "var(--c-bg-1)", color: "var(--c-accent)", border: "1px solid var(--c-border-2)" }}>{key}</kbd>
            </div>
          ))}
        </div>
      </div>
