...
export function BrowserApprovalCard({ message, timestamp }: Props) {
  ...
  const handleResolve = useCallback(async (status: "approved" | "denied") => {
      ...
      <div className="flex items-center gap-1.5 py-1 px-2">
        ...
        <span
          ...
          style={{
            ...
            color: resolved === "approved" ? "var(--c-success, #34d399)" :
                   resolved === "denied" ? "var(--c-danger-soft, #f87171)" :
                   "var(--c-warning, #fbbf24)",
            border: "1px solid var(--c-border-2)",
          }}
        >
          <span>{resolved === "approved" ? "\u2713" : resolved === "denied" ? "\u2717" : "\uD83D\uDD10"}</span>
          <span>{resolved === "approved" ? "Approved" : resolved === "denied" ? "Denied" : "Approval needed"}</span>
        </span>
        {timestamp && (
      ...
      >
        ...
        >
          <span className="font-medium text-[11px]" style={{ color: "var(--c-text-3)" }}>
            {ACTION_ICON[action] || "\uD83C\uDF10"} {action.replace("browser_", "").replace("_", " ")}
          </span>
          <span
        ...
        <div className="px-3 py-2 space-y-1">
          ...
          {target && (
            <div className="text-[10px] px-2 py-1 rounded" style={{ background: "var(--c-bg-2)", color: "var(--c-text-3)" }}>
              {target.length > 60 ? target.slice(0, 60) + "..." : target}
            </div>
          )}
          {reason && (
            <div className="text-[11px]" style={{ color: "var(--c-text-2)" }}>{reason}</div>
          )}
        </div>

        {/* Action buttons */}
        {!resolved && (
          <div className="px-3 py-2 flex gap-2" style={{ borderTop: "1px solid var(--c-border-2)" }}>
            <button
              onClick={() => handleResolve("approved")}
              disabled={resolving}
              className="px-3 py-1 rounded-md text-[11px] font-medium transition-all hover:opacity-90"
              style={{
                background: "var(--c-success, #34d399)", color: "#fff", border: "none",
            ...
            <button
              ...
              className="px-3 py-1 rounded-md text-[11px] font-medium transition-all hover:opacity-90"
              style={{
                background: "var(--c-danger-soft, #f87171)", color: "#fff", border: "none",
                cursor: resolving ? "not-allowed" : "pointer", opacity: resolving ? 0.6 : 1,
              }}
            >
              {resolving ? "..." : "Deny"}
            ...
            {approvalId && (
              <a
                ...
                style={{
                  background: "var(--c-bg-2)", color: "var(--c-text-4)",
        ...
        {resolved && (
          <div
            ...
            style={{
              color: resolved === "approved" ? "var(--c-success, #34d399)" : "var(--c-danger-soft, #f87171)",
              borderTop: "1px solid var(--c-border-2)",
            }}
          >
            {resolved === "approved" ? "Browser action approved — executing" : "Browser action denied — cancelled"}
          </div>
        )}
      </div>
    </div>
