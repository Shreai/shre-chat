...
export function PlanChecklist({ projectId, subtaskCount, timestamp }: PlanChecklistProps) {
  ...
  const isApproved =
    approved || plan?.status === 'approved' || plan?.status === 'executing' || isComplete;

  // Progress percentage
  const progressPct =
    totalTasks > 0 ? Math.round(((completedCount + failedCount) / totalTasks) * 100) : 0;
...
function TaskRow({ task }: { task: PlanTask }) {
  return (
    ...
    >
      ...
      {/* Status label */}
      <span className="text-[9px] flex-shrink-0" style={{ color: STATUS_COLOR[task.status] }}>
