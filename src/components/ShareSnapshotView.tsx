...
interface ShareSnapshotViewProps {
  snapshot: { title: string; messages: ChatMessage[]; model: string | null; createdAt: string } | null;
  loading: boolean;
  error: string | null;
}
...
export function ShareSnapshotView({
  ...
  loading,
  error,
}: ShareSnapshotViewProps) {
  if (!snapshot && !loading && !error) return null;

  return (
    <div className="flex-1 flex flex-col h-full min-w-0 relative">
      {/* Shared view header */}
      <div className="px-4 py-3 flex items-center gap-3 shrink-0" style={{ background: "var(--c-bg-2)", borderBottom: "1px solid var(--c-border-1)" }}>
        <a
          href="/"
        ...
        <div className="flex-1 min-w-0">
          ...
          {snapshot?.createdAt && (
            <div className="text-[10px]" style={{ color: "var(--c-text-5)" }}>
              Shared {new Date(snapshot.createdAt).toLocaleDateString()} &middot; {snapshot.messages.length} messages
              {snapshot.model ? ` \u00b7 ${snapshot.model}` : ""}
            </div>
          )}
        </div>
        <div className="text-[9px] px-2 py-1 rounded-full" style={{ background: "rgba(59,130,246,0.1)", color: "var(--c-info-soft)", border: "1px solid rgba(59,130,246,0.2)" }}>
          Read-only
        </div>
      </div>
      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-4 py-6">
        {loading && (
          <div className="text-center py-20" style={{ color: "var(--c-text-4)" }}>
            <span className="inline-block h-5 w-5 rounded-full border-2 border-t-transparent animate-spin mb-2" style={{ borderColor: "var(--c-accent)", borderTopColor: "transparent" }} />
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            {snapshot.messages.map((msg, i) => (
              ...
              >
                ...
                >
                  {msg.role !== "user" ? (
                    <Markdown remarkPlugins={[remarkGfm]}>{msg.content.replace(/<think>[\s\S]*?<\/think>\s*/g, "")}</Markdown>
                  ) : (
                    msg.content
                  )}
                </div>
