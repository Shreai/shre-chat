...
export interface ClaudeToolEvent {
  type: "delta" | "tool_start" | "tool_result" | "claude_result" | "session_start" | "session_end" | "status" | "error";
  text?: string;
...
export function ClaudeToolView({ toolCall, isStreaming }: ClaudeToolViewProps) {
  ...
  const { textOutput, toolUsages, meta } = useMemo(() => {
    ...
    let text = "";
    const tools: Array<{ name: string; input?: Record<string, unknown>; result?: string; isError?: boolean }> = [];
