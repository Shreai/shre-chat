...
export function getStoredAuth(): AuthState | null {
  ...
  } catch (err) { console.debug("loadPersistedAuth JSON parse", err); }
  return null;
}
...
export function useAuth(devBypass: boolean) {
  ...
  const [pendingWorkspaceSelection, setPendingWorkspaceSelection] = useState<PendingWorkspaceSelection | null>(null);
  ...
  useEffect(() => {
    ...
    if (stored) {
      fetch("/api/auth/check", {
        ...
        if (!r.ok) {
          ...
          } catch (err) { console.debug("auth check workspace parse", err); }
    ...
    function tryGateSSO() {
      fetch("/api/auth/gate-sso").then(async (r) => {
  ...
  const handleLogin = useCallback((token: string, user: AuthUser, loginData?: any) => {
    ...
    if (redirect) {
      ...
      } catch (err) { console.debug("redirect URL parse failed", err); }
    }
  }, []);
  ...
  const handleWorkspaceSelected = useCallback(async (workspaceId: string) => {
    ...
    } catch (err) { console.warn("workspace selection failed", err); }
  ...
  const handleWorkspaceSwitch = useCallback(async (workspaceId: string) => {
    ...
    } catch (err) { console.warn("workspace switch failed", err); }
  }, [authState]);
