  ...
  const handleSendReply = useCallback(async () => {
    ...
    if (slashMatch) {
      ...
      const participant = activeThread.participants.find(
        (p) =>
          p.name.toLowerCase().startsWith(name) || p.email.split('@')[0].toLowerCase() === name,
      );
      if (participant) {
    ...
    effectiveText = effectiveText.replace(/@(\w+)/g, (match, name) => {
      const p = activeThread.participants.find((pp) =>
        pp.name.toLowerCase().startsWith(name.toLowerCase()),
      );
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!loading && !error && threads.length === 0 && (
            <div className="p-8 text-center text-sm" style={{ color: 'var(--c-text-4)' }}>
              No emails found
            </div>
          )}
          {threads.map((t) => (
            ...
            >
              <div className="flex items-start gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div
                    className={`text-xs truncate ${t.unread ? 'font-medium' : ''}`}
                    style={{ color: t.unread ? 'var(--c-text-2)' : 'var(--c-text-3)' }}
                  >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1 mt-0.5">
                ...
                {activeThread.participants.length > 5 && (
                  <span className="text-[10px] ml-1" style={{ color: 'var(--c-text-4)' }}>
                    +{activeThread.participants.length - 5}
                  </span>
                )}
                <button
              ...
              {activeThread.participants.map((p) => (
                ...
                >
                  <span
                    ...
                    style={{ background: colorForEmail(p.email) }}
                  >
                    {initials(p.name || p.email)}
                  </span>
                ...
                <div key={msg.id}>
                  ...
                  <div className={`flex gap-2.5 ${msg.isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    <div className={`max-w-[75%] min-w-[200px] ${msg.isMe ? 'ml-auto' : ''}`}>
                      {/* Sender info */}
                      <div
                        className={`flex items-center gap-2 mb-1 ${msg.isMe ? 'justify-end' : ''}`}
                      >
                      ...
                      {/* Actions */}
                      <div
                        className={`flex items-center gap-1 mt-1 ${msg.isMe ? 'justify-end' : ''}`}
                      >
                        <button
                          onClick={() => {
            ...
            >
              ...
              <button
                onClick={() => {
                  ...
                  if (replyMode === 'direct') setReplyTo(null);
                }}
              ...
              {extraRecipients.map((r) => (
                ...
                >
                  +{r.split('@')[0]}
                  <button
                    onClick={() => setExtraRecipients((prev) => prev.filter((e) => e !== r))}
              ...
              <div className="mb-1 flex flex-wrap gap-1">
                {replyHint.map((p) => (
                  <button
                    ...
                    onClick={() => {
                      setReplyText(
                        replyText.replace(/\/\w*$/, `/${p.name.split(' ')[0].toLowerCase()} `),
                      );
                      replyRef.current?.focus();
                    }}
