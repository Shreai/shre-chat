...
// ── Extracted modules ──
import { stripMd, detectCmd, detectAgentSwitch, type AgentOption, type Turn, type VoiceShortcut } from "./voice/voice-utils";
...
export default function VoiceAssistant({ open, onClose, messages, agentName, agentEmoji, agentId, ttsVoice, agents, onSwitchAgent, onVoiceTurn, openclawMode }: Props) {
  ...
  useEffect(() => {
    ...
    } catch (err) { console.debug("voice turn restore from sessionStorage", err); }
  ...
  useEffect(() => {
    if (turns.length > 0) {
      ...
      } catch (err) { console.debug("voice turn persist to sessionStorage", err); }
  ...
  const vad = useVAD({
    ...
    onSpeechStart: useCallback(() => { dispatchRef.current({ type: "SPEECH_DETECTED" }); }, []),
    onSpeechEnd: useCallback(() => { dispatchRef.current({ type: "SPEECH_ENDED" }); }, []),
  });
  ...
  const cleanup = useCallback(() => {
    ...
    if (recRef.current) { try { recRef.current.abort(); } catch (err) { console.debug("rec abort cleanup", err); } recRef.current = null; }
    vad.destroy();
    ...
    if (mediaRecorderRef.current) {
      if (mediaRecorderRef.current.state !== "inactive") { try { mediaRecorderRef.current.stop(); } catch (err) { console.debug("mediaRecorder stop cleanup", err); } }
  ...
  const speak = useCallback(
    createSpeak({ ttsVoice, activeRef, ttsAbortRef, ttsAudioRef, mediaStreamRef, phaseRef, dispatch, vad }),
    ...
    try {
      ...
      return await new Promise<string>((resolve) => {
        ...
        sendChatMessage(
          ...
          callbacks, signal, voiceSessionIdRef.current || undefined,
          undefined, undefined, openclawMode,
        ).catch((err) => {
          clearTimeout(timeoutId);
  ...
  const transcribeWithWhisper = useCallback(async (audioBlob: Blob): Promise<string> => {
    try {
      ...
      const res = await fetch("/api/transcribe", { method: "POST", body: formData, signal: ctrl.signal });
  ...
  const startRecording = useCallback(async () => {
    if (mediaRecorderRef.current && mediaRecorderRef.current.state !== "inactive") {
      try { mediaRecorderRef.current.stop(); } catch (err) { console.debug("mediaRecorder stop before restart", err); }
      mediaRecorderRef.current = null;
    }
  ...
  const tryVoiceCommand = useCallback(async (text: string): Promise<{ spoken: string; mib007Link?: string; action?: string } | null> => {
    try {
      ...
      const data = await res.json();
      if (data.action === "clarify" && data.spoken) return { spoken: data.spoken, action: "clarify" };
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      const stream = await acquireMic();
      if (!stream) { dispatch({ type: "ERROR", message: "Microphone access denied. Allow mic access in browser settings." }); return; }
      await startRecording();
      try { vad.start(stream); } catch (e) { console.warn("[voice] VAD start failed:", e); }

      const SR = window.SpeechRecognition || (window as any).webkitSpeechRecognition;
      if (SR) {
        if (recRef.current) { try { recRef.current.abort(); } catch (err) { console.debug("rec abort before new", err); } }
        const rec = new SR();
        rec.continuous = true;
        ...
        rec.onend = () => {
          if (activeRef.current && phaseRef.current === "listening") {
            try { rec.start(); } catch (err) { console.debug("SR restart failed", err); }
          }
        };
        rec.onerror = (e: any) => {
        ...
        try { rec.start(); recRef.current = rec; } catch (err) { console.debug("SR start failed, Whisper still active", err); }
      }
    } catch (err) {
  ...
  const finishListening = useCallback(async () => {
    ...
    try {
      ...
      if (recRef.current) { try { recRef.current.abort(); } catch (err) { console.debug("rec abort on stop", err); } recRef.current = null; }
      try { vad.stop(); } catch (err) { console.debug("VAD stop", err); }
      ...
      if (!whisperText || !activeRef.current || (phaseRef.current as VoicePhase) !== "transcribing") {
        if (activeRef.current && (phaseRef.current as VoicePhase) === "transcribing") dispatch({ type: "INTERRUPT" });
        return;
      }
      processUserInput(whisperText);
    } catch (err) {
      console.error("[voice] finishListening crashed:", err);
      if (activeRef.current) dispatch({ type: "ERROR", message: "Voice processing error. Tap to retry." });
    }
  ...
  useEffect(() => {
    ...
    if (allTurns.length >= 4) {
      fetch("/api/voice-session-summary", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ turns: allTurns.map(t => ({ role: t.role, text: t.text })), agentId }),
      }).catch(() => { void 0; });
    }
  }, [open, agentId]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved && Date.now() - lastSaved < 30 * 60 * 1000) {
        const parsed = JSON.parse(saved) as Turn[];
        if (parsed.length > 0) { setTurns(parsed); turnHistoryRef.current = parsed; }
        else { setTurns([]); turnHistoryRef.current = []; }
      } else { setTurns([]); turnHistoryRef.current = []; }
    } catch (err) {
      console.debug("voice session restore failed", err);
      setTurns([]); turnHistoryRef.current = [];
    }
    dispatch({ type: "OPEN" });
  ...
  const handleOrbTap = useCallback(() => {
    ...
    } else if (phase === "transcribing") { whisperAbortRef.current?.abort(); dispatch({ type: "INTERRUPT" }); }
    else if (phase === "thinking") { aiAbortRef.current?.abort(); dispatch({ type: "INTERRUPT" }); }
    else if (phase === "error") { dispatch({ type: "RETRY" }); }
  ...
  const orbGlow = phase === "ready"
    ...
    : phase === "listening"
      ? speechActive ? "0 0 50px 15px rgba(239, 68, 68, 0.5)" : "0 0 25px 8px rgba(239, 68, 68, 0.25)"
      : phase === "speaking" ? "0 0 30px 10px rgba(34, 197, 94, 0.25)"
        : phase === "thinking" || phase === "transcribing" ? "0 0 25px 8px rgba(59, 130, 246, 0.2)"
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col" style={{ background: "linear-gradient(180deg, #0a1628 0%, #0d1f3c 50%, #0a1628 100%)" }}>
      ...
      <div className="flex items-center justify-between px-5 pt-5 pb-2" style={{ paddingTop: "calc(1.25rem + env(safe-area-inset-top, 0px))" }}>
        ...
        >
          ...
          {agents && agents.length > 1 && (
            <svg className="h-3.5 w-3.5" style={{ color: "rgba(255,255,255,0.4)", transform: agentPickerOpen ? "rotate(180deg)" : "" }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9" /></svg>
          )}
        </button>
        <div className="flex items-center gap-2">
          ...
          >
            <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
            {proactiveMode ? "Live" : "Alerts"}
            {proactiveMode && notifConnected && <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" />}
            {proactiveMode && pendingNotifs.length > 0 && <span className="ml-0.5 text-[9px] bg-red-500 text-white rounded-full px-1">{pendingNotifs.length}</span>}
          </button>
          <button
            onClick={onClose}
      ...
      {agentPickerOpen && agents && agents.length > 1 && (
        <div className="mx-5 mb-2 rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }}>
          {agents.filter((a) => a.id !== agentId).slice(0, 8).map((a) => (
            <button key={a.id} className="w-full flex items-center gap-3 px-4 py-2.5 text-left active:bg-white/10 transition-colors" style={{ color: "rgba(255,255,255,0.85)", borderBottom: "1px solid rgba(255,255,255,0.04)" }}
              onClick={() => { setAgentPickerOpen(false); onSwitchAgent?.(a.id); }}>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-3 space-y-3" style={{ minHeight: 0 }}>
        {turns.map((t, i) => (
          <div key={i} className={`flex ${t.role === "user" ? "justify-end" : "justify-start"} animate-fadeIn`}>
            <div className="max-w-[85%] rounded-2xl px-4 py-2.5 text-[14px] leading-relaxed" style={{
              ...
              {t.mib007Link && (
                <button onClick={() => window.open(`${window.location.hostname !== "localhost" ? "https://app.nirtek.net" : "https://localhost:5520"}${t.mib007Link}`, "_blank")}
                  className="block mt-1.5 text-[12px] opacity-70 hover:opacity-100 transition-opacity underline" style={{ color: "rgba(96, 165, 250, 0.9)" }}>
                  View in MIB007 &rarr;
                </button>
              )}
            </div>
      ...
      {briefingPlaying && (
        <div className="absolute inset-0 z-10 flex items-end justify-center pb-8 cursor-pointer" onClick={skipBriefing} role="button" aria-label="Skip briefing">
          <span className="text-[11px] px-4 py-2 rounded-full" style={{ background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.5)" }}>Tap anywhere to skip briefing</span>
        </div>
      )}
      ...
      {shortcuts.length > 0 && state.phase === "ready" && (
        <div className="px-5 py-1.5 flex flex-wrap gap-2 justify-center">
          {shortcuts.map(s => (
            <button key={s.id} className="text-[11px] px-3 py-1.5 rounded-full active:scale-95 transition-transform"
      ...
      <div className="px-5 min-h-[3rem] flex items-center justify-center">
        {transcript && phase === "listening" ? (
          <p className="text-sm text-center max-w-md leading-relaxed" style={{ color: "rgba(255,255,255,0.55)" }}>&ldquo;{transcript}&rdquo;</p>
        ) : phase === "error" ? (
      ...
      <div className="flex flex-col items-center pb-6" style={{ paddingBottom: "calc(1.5rem + env(safe-area-inset-bottom, 0px))" }}>
        <button onClick={handleOrbTap} className="relative h-24 w-24 rounded-full flex items-center justify-center active:scale-90"
          style={{
            background: phase === "ready" ? "radial-gradient(circle, rgba(255,255,255,0.15) 0%, rgba(255,255,255,0.08) 100%)"
              : phase === "listening" ? "radial-gradient(circle, rgba(239,68,68,0.85) 0%, rgba(220,38,38,0.95) 100%)"
              : phase === "speaking" ? "radial-gradient(circle, rgba(34,197,94,0.8) 0%, rgba(22,163,74,0.9) 100%)"
              : phase === "thinking" || phase === "transcribing" ? "radial-gradient(circle, rgba(59,130,246,0.8) 0%, rgba(37,99,235,0.9) 100%)"
              : phase === "error" ? "radial-gradient(circle, rgba(239,68,68,0.6) 0%, rgba(185,28,28,0.8) 100%)"
              : "radial-gradient(circle, rgba(107,114,128,0.6) 0%, rgba(75,85,99,0.8) 100%)",
            boxShadow: orbGlow, transform: `scale(${orbScale})`, transition: "transform 150ms ease-out, box-shadow 150ms ease-out",
          }}
        ...
        >
          {phase === "ready" ? (
            <svg className="h-10 w-10 text-white/80 drop-shadow" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z" /><path d="M19 10v2a7 7 0 0 1-14 0v-2" /><line x1="12" y1="19" x2="12" y2="23" /><line x1="8" y1="23" x2="16" y2="23" /></svg>
          ) : phase === "listening" ? (
            <svg className="h-9 w-9 text-white drop-shadow" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="2" /></svg>
          ) : phase === "speaking" ? (
            <svg className="h-9 w-9 text-white drop-shadow" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5" fill="white" /><path d="M15.54 8.46a5 5 0 0 1 0 7.07" /><path d="M19.07 4.93a10 10 0 0 1 0 14.14" /></svg>
          ) : phase === "error" ? (
            <svg className="h-9 w-9 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>
          ) : (
        ...
        {phase === "ready" && <p className="text-[10px] mt-1.5 text-center max-w-[220px] leading-relaxed" style={{ color: "rgba(255,255,255,0.35)" }}>Tap to talk</p>}
        {phase === "listening" && <p className="text-[10px] mt-1.5 text-center max-w-[220px] leading-relaxed" style={{ color: "rgba(255,255,255,0.25)" }}>Tap to send</p>}
        {phase === "transcribing" && <p className="text-[10px] mt-1.5" style={{ color: "rgba(255,255,255,0.25)" }}>Tap to cancel</p>}
        {phase === "speaking" && <p className="text-[10px] mt-1.5" style={{ color: "rgba(255,255,255,0.25)" }}>Tap to interrupt</p>}
        {phase === "thinking" && <p className="text-[10px] mt-1.5" style={{ color: "rgba(255,255,255,0.25)" }}>Tap to cancel</p>}
        {phase === "error" && <p className="text-[10px] mt-1.5" style={{ color: "rgba(255,255,255,0.25)" }}>Tap to retry</p>}
      </div>
