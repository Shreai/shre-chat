...
test.describe("Preview — rendering & file type support", () => {
  ...
  test("Library shows multiple entries", async ({ page }) => {
    await page.evaluate(() => {
      const entries = [
        { id: "test_1", title: "report.html", html: "<h1>HTML</h1>", savedAt: Date.now(), type: "html" },
        { id: "test_2", title: "data.csv", html: "a,b\n1,2", savedAt: Date.now() - 1000, type: "csv" },
        { id: "test_3", title: "config.json", html: '{"key":"val"}', savedAt: Date.now() - 2000, type: "json" },
      ];
